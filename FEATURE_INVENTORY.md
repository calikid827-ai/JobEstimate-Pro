...
- Next active smart-estimator task: Phase 8D follow-up audit of remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `profitLeakDetector` and photo-estimate decision text. Prompts, `effectiveScopeChange`, `result.text`, pricing policy, photo pricing behavior, and route/API shape should not change without a scoped review.
...
1. Audit remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `profitLeakDetector` and photo-estimate decision text, as the next Phase 8D follow-up without changing prompts, `effectiveScopeChange`, `result.text`, pricing policy, photo pricing behavior, or route/API shape until the raw-scope risk is mapped.
