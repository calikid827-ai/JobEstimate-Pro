...
- Customer Scope Drift electrical masking polish is complete. Painting-prep wording around outlets, switches, cover plates, and electrical fixtures-to-remain is treated as context-only, so phrases such as masking tape applied to trim/outlets/switches, mask/protect/cover outlets and switches, remove and reinstall outlet covers for painting only, and electrical fixtures to remain no longer create unsupported electrical drift or auto-open Customer Output Readiness. True unsupported electrical work still warns for replace/install outlets, move switches, repair wiring, electrical rough-in, run new wire, add circuit, panel work, and install light fixtures. This remains UI/client review-only and does not change backend behavior, API shape, saved estimates, PDFs/customer output, pricing, prompts, `result.text`, materials, deterministic engines, billing/auth, Smart Questions authority, or Evidence Authority exposure.
