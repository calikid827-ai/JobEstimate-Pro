...
- Shared EstimatorScopeFacts helper:
  ...
  - Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, backend Estimate Defense, backend missedScopeDetector, route-level display-only Scope-to-Price X-Ray / area confirmation diagnostics, `buildMaterialsList` confirmItems/notes, selected `materialsList.items` conditional gates, customer-facing trade coordination append gating, schedule/rationale multi-trade text gating, and route display multi-trade diagnostic gating now consume EstimatorScopeFacts where safe. Remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `buildEstimateExplanation`, `profitLeakDetector`, and photo-estimate decision text, plus broader route diagnostics, `scopeSplitter`, and pricing prep are not fully migrated/audited yet.
...
- Next active smart-estimator task: Phase 8D follow-up audit of remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `buildEstimateExplanation`, `profitLeakDetector`, and photo-estimate decision text. Prompts, `effectiveScopeChange`, `result.text`, pricing policy, and route/API shape should not change without a scoped review.
...
1. Audit remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `buildEstimateExplanation`, `profitLeakDetector`, and photo-estimate decision text, as the next Phase 8D follow-up without changing prompts, `effectiveScopeChange`, `result.text`, pricing policy, or route/API shape until the raw-scope risk is mapped.
