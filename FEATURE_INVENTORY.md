...
- The pure readiness helper supports `Ready to send`, `Review before sending`, and `Finish estimate first`. Because Proposal renders only after a result exists, Finish estimate first is a safe helper fallback rather than a new pre-result surface. Readiness uses existing unsupported-scope drift, Customer Output Readiness items, Estimator Review Summary status, PriceGuard level/score, unanswered high-priority Smart Questions, and plan/photo warnings. It adds no saved readiness field, localStorage key, or server persistence and does not change pricing authority, pricing, `result.text`, PDF content, saved data shapes, or `/api/generate`.
