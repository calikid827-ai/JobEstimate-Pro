...
- Shared EstimatorScopeFacts helper:
  ...
  - Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, backend Estimate Defense, backend missedScopeDetector, route-level display-only Scope-to-Price X-Ray / area confirmation diagnostics, and `buildMaterialsList` confirmItems/notes now consume EstimatorScopeFacts where safe. `materialsList.items` generation, broader materials generation, `scopeSplitter`, and pricing prep are not fully migrated/audited yet.
...
- Backend materials diagnostics now consume EstimatorScopeFacts where safe for `buildMaterialsList` confirmItems/notes. `route.ts` passes `scopeFacts` into `buildMaterialsList`, and route display diagnostics helper logic filters confirmation items and notes for excluded patch/texture/drywall context, by-others plumbing/electrical context, owner/customer-supplied fixture/material boundaries, flooring protection and existing-to-remain context, tile trim vs carpentry/base trim context, and true mixed materials note gating. `materialsList.items` generation is intentionally not migrated yet.
- The UI-side estimator stack plus backend Estimate Defense, missedScopeDetector, route-level display diagnostics, and materials diagnostics now have EstimatorScopeFacts coverage across typed-scope normalization, Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, display-only Estimate Defense diagnostics, warning-only missed-scope diagnostics, display-only X-Ray / area confirmation diagnostics, and materials confirmItems/notes where safe.
- Remaining route-level `materialsList.items` generation, broader materials generation, `scopeSplitter`, pricing prep, and any remaining backend route diagnostics are not fully migrated/audited to EstimatorScopeFacts yet. Customer-Facing Scope / `result.text` remains preserved and was not broadly rewritten.
...
- Next active smart-estimator task: Phase 8C audit of remaining route-level `materialsList.items` generation for raw scope parsing. This should be audit/planning only first because item generation is customer-visible and higher risk than confirmation notes.
...
1. Audit remaining route-level `materialsList.items` generation as Phase 8C of the shared scope-understanding architecture, without implementing changes until the customer-visible item-generation risk is mapped.
