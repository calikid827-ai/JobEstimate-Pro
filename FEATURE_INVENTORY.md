...
- Crew Planning is an estimator-only, `data-no-print`, client-side planning panel after Estimated Schedule / schedule editor and before Estimator Review Summary. It is not sent to `/api/generate`, not saved to history/localStorage, not included in PDFs/customer output, and `affectsPricing: false`. It now infers painting when selected trade is General Renovation but typed scope is painting-heavy, and its missing crew-day/duration copy uses visit-aware labels instead of repeated “Needs schedule.”
- Tax controls.
- Deposit controls.
