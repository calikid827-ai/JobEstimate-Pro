...
- Customer Scope Drift final context-wording polish is complete. Painting-prep floor/flooring masking/protection, outlet/switch masking, cover-plate/outlet-cover handling, generated outlet-cover coordination, safely remove/reinstall wording, wiring/device/circuit unaffected language, and “handle outlet covers safely / prevent interference with existing wiring” are treated as context-only and no longer create unsupported drift or auto-open Customer Output Readiness. True flooring work still warns for install/replace/repair/remove/level/underlayment/flooring demolition, and true electrical/circuit work still warns for outlet/switch/device/fixture/wiring/circuit/panel/rough-in work. Crew Planning avoids the multi-trade note for masking/protection/cover-plate-only prep while preserving it for true typed multi-trade work. Final desktop/mobile Playwright screenshot QA passed with `test12345@gmail.com` and artifacts in `/tmp/jobestimatepro-wiring-interference-final-qa`; Customer Scope Drift tests passed 77/77. This remains UI/client review-only and does not change backend behavior, API shape, saved estimates, PDFs/customer output, pricing, prompts, `result.text`, materials, deterministic engines, billing/auth, Smart Questions authority, or Evidence Authority exposure.
...
- Generated estimate result hierarchy now prioritizes the primary send workflow while reducing duplicate review messaging: Customer-Facing Scope, Pricing/PDF, Schedule, compact Estimator Review Summary, Quick Clarifications, compact Customer Output Readiness details, collapsed `Estimator review details`, and collapsed `Estimator Diagnostics`. Unsupported trade drift warnings remain visible above Customer-Facing Scope and can auto-open Customer Output Readiness when critical. PriceGuard Review remains inside collapsed estimator review details, and nested diagnostics such as Scope-to-Price X-Ray, Materials List, Area Scope Breakdown, and Profit Protection are collapsed by default.
- Crew Planning is an estimator-only, `data-no-print`, client-side planning panel after Estimated Schedule / schedule editor and before Estimator Review Summary. It is not sent to `/api/generate`, not saved to history/localStorage, not included in PDFs/customer output, and `affectsPricing: false`. It now infers painting when selected trade is General Renovation but typed scope is painting-heavy, uses visit-aware labels instead of repeated “Needs schedule,” and shows a compact estimator-only planning note when typed scope clearly includes multiple trades. Protection-only floor wording such as protect floors with drop cloths, floor protection, flooring protected with drop cloths, and drop cloths over flooring does not trigger the multi-trade note; typed work such as painting plus flooring or painting plus outlets does.
- Tax controls.
- Deposit controls.
