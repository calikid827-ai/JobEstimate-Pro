...
- Backend scope-boundary filtering now uses included-work scope text for split-scope detection, mixed-renovation detection, and PriceGuard anchor eligibility. Excluded/by-others/protection/coordination-only/existing-condition clauses no longer create false backend split-scope trades. The Case 7A painting scope no longer triggers General Renovation, `flooring_only_v1`, flooring split scopes, or flooring materials from `floor protection` / adjacent-trade exclusions, while true mixed painting + flooring still works. `result.text` remains preserved.
...
- Backend scope splitter included-work filtering for split-scope, mixed-renovation, and anchor eligibility paths. This prevents excluded/protection/coordination-only language from creating false Scope-to-Price X-Ray trades, false `flooring_only_v1` anchors, or false flooring materials, while preserving true mixed-scope behavior.
...
- Next active smart-estimator audit: cross-trade backend scope-boundary regression review for by-others, owner-supplied, protection, coordination, and existing-condition language.
...
1. Run cross-trade backend scope-boundary regression review before expanding the Case 7A included-work filtering pattern.
