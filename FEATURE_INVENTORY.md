...
- Photo Intelligence Actions V1 is complete across evidence-bound model V1A (`2e7be1a`) and runtime integration V1B (`9832108`):
  - One shared request-selection path supplies the exact ordered photo evidence used for Generate and for a session-only evidence fingerprint. Request order, image identity, name, room tag, shot type, note, and reference metadata contribute to evidence identity without adding an API field or persisted fingerprint.
  - Freshness is classified as current, stale, unverified, or no-photo. Only verified-current evidence can produce photo actions. Add/remove/replace/order/metadata/reference changes suppress prior actionability, while loaded legacy/saved readbacks remain unverified and non-actionable until a successful new Generate establishes provenance. A verified no-photo generation produces no actions; adding request-eligible evidence afterward requires regeneration. Uploads excluded by the existing request payload-size rules are outside selected evidence and do not create false staleness merely by remaining in the upload list.
  - Runtime eligibility is painting-only and exact-match-only for structured ceiling boundary and trim/baseboard/casing boundary flags. Suggested additions/notes, observations, captions, generic AI prose, quantities, square footage, measurements, reference-scaled takeoff, damage/repair/prep/protection/access/demo/material/equipment/coordination findings, and mixed plan/photo conclusions remain non-actionable.
  - Eligible actions enter the existing Scope Decisions experience inside Review Before Sending. Choices remain Include, Exclude, By others, and Needs site confirmation with canonical deterministic wording. Ordinary Scope Decisions reserve the same trade/subject boundary first, existing priority and the three-decision cap remain, and stale applied photo records cannot resurrect an action control. Already-applied wording remains ordinary contractor-owned typed scope and is not automatically removed when photo evidence later changes.
  - Contractor review and explicit Apply change only typed scope. The generated result then becomes stale and manual Generate remains required; photo Apply does not directly change price, Rate Card, labor totals, production rates, or proposal/PDF output.
  - Changed request-eligible evidence uses canonical Send Readiness `Regenerate before sending`; no separate photo-readiness verdict or sixth Command Center section was added. The typed-scope Scope updated notice remains specific to typed-scope changes.
  - The former Proposal Smart Scope Assist `Apply to Scope Only`, `Regenerate with Suggestions`, and direct regeneration path are retired. Underlying `photoAnalysis`/`photoScopeAssist`, Smart Questions review input, Estimator Review, Photo Intelligence readback, Advanced Diagnostics, and saved readbacks remain available.
  - Provenance and photo-applied source association are ephemeral React state only. No localStorage key, saved estimate/history field, API/server/database field, schema migration, or historical evidence reconstruction was added.
- Plan upload:
  - PDF/image support
...
- Customer Output Readiness provides estimator-only source details for PDF/download/customer-output readiness, summarizing unsupported trade wording, weak/review-only plan evidence, scope clarity, assumptions/exclusions, estimator risk notes, and send-readiness concerns without changing customer output or pricing.
  - Details are deduped across readiness items, capped at 2 per item, and the panel remains capped at 6 items. It now sits below `EstimatorReviewSummaryPanel` and Scope Decisions as compact/collapsed details so the summary remains the primary review hub.
...
- Smart Questions V1 + Confirmed Answers V2-lite remains the client-side structured review-question layer. Smart Questions V2A still provides classification-only future statuses and leaves `pricingAuthoritative` and `pricingEligibleNow` false. The former Quick Clarifications interaction was replaced by Actionable Scope Decisions V1 in `7c5ac91`; confirmed-answer state is not restored, and Evidence Authority remains internal/debug-only.
- Actionable Scope Decisions V1 appears in the former Quick Clarifications position inside Review Before Sending and does not add a sixth Command Center section or another diagnostics panel. It renders zero to three contractor-facing decisions from safely structured metadata. Runtime actionability remains allowlist-only: structured material responsibility plus Photo Intelligence Actions V1's exact verified-current painting ceiling and trim/baseboard/casing flags. Arbitrary warning, diagnostic, plan input, generic photo/AI prose, Scope X-Ray, PriceGuard, missed-scope, quantity, or measurement signals cannot manufacture a deterministic subject or sentence. Plans remain non-actionable in this workflow; doors/frames, closets, repairs, access equipment, protection, furniture moving, occupied-area coordination, photo quantities, and measurements remain deferred.
- Each decision requires a contractor choice, exact deterministic sentence preview, and explicit `Apply to typed scope`. Selection alone changes nothing. Apply changes only the live typed-scope input, does not call Generate, and changes Send Readiness to `Regenerate before sending`. Until successful manual regeneration, proposal text, pricing/labor display, Price & Profit readbacks, estimate confidence, Customer Output Readiness, customer scope drift, Scope Decisions, Crew Planning, Field Handoff, Active Job Summary, PDF, and copied proposal text remain tied to the exact generated-scope snapshot.
- Scope Decision choices, feedback, and wording ownership are local React state only. No localStorage key, saved estimate/history/job/invoice/approval shape, or server-backed decision state was added. Existing exact normalized line/sentence wording is treated as already present without being claimed. Feature-owned replacement requires an unchanged post-Apply scope snapshot and the exact recorded sentence range; any manual textarea edit fails closed with ownership-loss feedback and never rewrites or auto-appends contractor prose. Owner-supplied wording states responsibility without asserting pricing treatment, and trusted quantity wording uses canonical helper-owned labels plus a matching fixed unit.
...
- Proposal Readiness Badge V1 (`37c8a14`) adds one compact contractor-facing Send Readiness summary inside the existing Proposal section, directly above or near the unchanged PDF/copy actions. It shows only `Send Readiness`, one status, one short reason, and optional `Review items`; it is not a sixth Command Center section or another review/reporting panel and does not duplicate Customer Output Readiness, PriceGuard Review, Smart Questions, or Advanced Diagnostics details.
- The pure readiness helper supports `Ready to send`, `Review before sending`, `Regenerate before sending`, and `Finish estimate first`. Because Proposal renders only after a result exists, Finish estimate first is a safe helper fallback rather than a new pre-result surface. Readiness uses existing unsupported-scope drift, Customer Output Readiness items, Estimator Review Summary status, PriceGuard level/score, every unresolved high-priority Smart Question, and plan/photo warnings. Actionable and review-only high-priority questions both block Ready; selection does not resolve them, and stale typed scope takes priority until regeneration. It adds no saved readiness field, localStorage key, or server persistence and does not change pricing authority, pricing, `result.text`, PDF content, saved data shapes, or `/api/generate`.
- `Review items` is `data-no-print` and scrolls only to the existing Review Before Sending section without opening Advanced Diagnostics, calling Generate, or mutating pricing, `result.text`, history, jobs, invoices, Rate Card, Saved Job Templates, Field Handoff, or Active Job Summary. Proposal PDF/copy actions remain enabled and unchanged.
- Severity follow-up `0afc82e` keeps Unsupported trade wording critical and preserves the scope-overpromise reason while removing the broader Customer-ready review row from the critical-label match. Customer-ready review can still produce `Review before sending` through readiness-item count or PriceGuard level/score, but it no longer produces `Customer-facing scope has a critical review item.` No Ready threshold, PriceGuard threshold/classification, helper priority, readiness-item generation, Estimator Review Summary, Smart Questions, or plan/photo behavior changed.
...
- Job Workflow Dashboard Polish V1 added a compact Active Job Summary at the top of the existing Job Workflow Command Center section without creating a sixth Command Center section. The five primary sections remain Proposal, Price & Profit, Schedule & Crew, Review Before Sending, and Job Workflow, with Jobs, Job Templates, Invoices, Saved Estimates, and Field Handoff still available below the summary.
- Active Job Summary derives contractor-facing workflow information from existing state only: active job, current loaded estimate only when it matches the summarized job, job details, workflow/pipeline status, contract value, approval status, invoice summary, recorded-cost workflow status, crew load, and Field Handoff readiness when available. The visible Actuals / Profit tile and selected-job partial-cost Profit Protection / live-margin readback were removed; this correction does not claim that unrelated profit or margin surfaces elsewhere were audited. It adds no localStorage key, server persistence, saved estimate/job/invoice data shape, pricing authority, PDF content, API response shape, or `/api/generate` behavior.
- Active Job Summary provides one practical next action derived from existing state, such as Create/select job, Copy approval link, Create deposit invoice, Create final/balance invoice, Open invoices, Open actuals, or Open Field Handoff. Summary actions reuse existing nearby workflow behavior or scroll to existing workflow surfaces; summary-only scroll/open actions do not call Generate and do not mutate pricing, `result.text`, history, jobs, or invoices.
- Actuals vs Estimate Feedback V1 is complete across V1A (`6f535d2`) and V1B (`910391d`). The pure helper resolves only the exact nonblank `Job.originalEstimateId`, ignores mismatched `actuals.jobId`, and compares exactly Labor, Materials, and Subs / other from the original saved estimate with contractor-recorded costs to date. It never falls back to an earlier/latest estimate, current generated pricing, invoices, approvals, revisions, or change orders.
- The contractor-facing `data-no-print` surface appears only inside the active Job Workflow card directly above the existing actual-cost editor. It supports no-linked-estimate, no-recorded-cost, partial-recorded-cost, and all-supported-costs-recorded-to-date states; uses `Not recorded`, Over estimate, Under estimate, On estimate, and Unplanned recorded cost wording; and states that it has not changed Rate Card or future estimates. It does not calculate actual profit, final margin, completion, labor/production efficiency, invoice or billing variance, or recommendations.
- Existing immediate actual-cost inputs and the `jobestimatepro_actuals_v1` shape remain unchanged. Actual-cost edits do not call Generate or mutate pricing, Rate Card, proposal/PDF content, saved estimate history, or invoices. `ResultCommandSection` is module-scoped to preserve native focus through rerenders, and the disclosure uses local React state only with no WeakMap, external cache, new storage key, or saved UI state.
- Field Handoff readiness/current estimate data is scoped to the summarized job so Active Job Summary does not offer Field Handoff or borrow contract/approval data for the wrong selected job.
...
- Full no-code end-to-end regression QA passed after `37c8a14 Add proposal readiness badge` using `test12345@gmail.com`. QA verified `/api/generate` returned 200; five Command Center sections rendered; Advanced Diagnostics stayed collapsed; Send Readiness appeared inside Proposal; Review items was scroll-only with no Generate or checked-state mutation; Proposal PDF/copy, Rate Card save/persist/apply, Active Job Summary and its cross-job Field Handoff guard, Field Handoff copy isolation, and Job Templates refresh/apply remained working; intended `data-no-print` controls were hidden; no console/page errors or regressions appeared; the working tree stayed clean; and the dev server stopped.
- Post-push regression QA passed after `7c5ac91 Add actionable scope decisions` using `test12345@gmail.com`. TypeScript passed, focused tests passed 54/54, both intentional Generate requests returned 200, exactly five Command Center sections remained, Scope Decisions rendered one safe material-responsibility decision without inferred unsupported subjects, generated-result readbacks stayed frozen before regeneration, stale readiness and fail-closed ownership behavior passed, PDF/copy/print/mobile checks passed, no console/page errors appeared, all test processes stopped, and the working tree stayed clean.
- Actuals vs Estimate Feedback V1 passed TypeScript, 40/40 focused tests, focused review, and browser regression using `test12345@gmail.com`. Exactly five Command Center sections remained; the comparison stayed inside the active Job Workflow card; real character-by-character Labor, Materials, Notes, and Subs / other input retained focus; no Generate request occurred during actual-cost edits; pricing, Rate Card, proposal, history, and invoice state remained unchanged; mobile and print behavior passed; and no console, React, hydration, or page errors appeared.
- Focused helper tests passed 10/10 with TypeScript and diff checks. Browser QA did not naturally observe `Ready to send`; an explicit painting fixture remained in review because existing upstream signals remained. Clean Ready remains covered by the pure helper, and a complete code-level painting case produced strong PriceGuard with no warning/risk rows. Focused QA after `0afc82e`, also using `test12345@gmail.com`, showed `Review before sending` with `4 pre-send items need review.`, no inaccurate critical wording, no extra Generate or checked localStorage mutation, working PDF/copy actions, no console/page errors, and a stopped dev server. That fixture did not contain Customer-ready review or unsupported scope drift; those branches were verified through code inspection and existing helper behavior.
...
- Completed: Photo Intelligence Actions V1 in `2e7be1a` and `9832108`. Evidence-bound verified-current painting actions now use Scope Decisions inside Review Before Sending; the parallel Smart Scope Assist action path is retired, stale/unverified evidence fails closed, and typed scope plus manual regeneration remain the only estimating-authority path. No photo pricing, quantity/measurement authority, persistence, API/schema change, or sixth section was added.
- Next product-intelligence feature: Plan Intelligence Workflow V1, followed by Contractor Learning System as a separately scoped phase.
...
1. Build Plan Intelligence Workflow V1 on the existing plan-intelligence backbone while preserving typed scope as the scope-control anchor.
2. Build Contractor Learning System only after reference-only feedback and evidence workflows are stable.
3. Continue focused real-world estimate, plan evidence, and workflow regression QA without broadening authority.
4. Continue focused `PRODUCT_AUDIT_BACKLOG.md` improvements without reopening completed Photo Intelligence Actions V1 authority boundaries.
