...
- Next active smart-estimator task: decide whether Phase 8D-6B should address photo-estimate decision behavior now that the path is characterized. Any implementation must be extra cautious because this path is pricing/policy-adjacent; do not change photo pricing behavior, `pricingAllowed`, blockers, confidence, confidenceBand, estimateMode, pricing policy, prompts, `effectiveScopeChange`, `result.text`, or route/API shape without a scoped review.
...
1. Decide whether Phase 8D-6B should address characterized photo-estimate decision false positives without changing photo pricing behavior, `pricingAllowed`, blockers, confidence, confidenceBand, estimateMode, pricing policy, prompts, `effectiveScopeChange`, `result.text`, or route/API shape without a scoped review.
