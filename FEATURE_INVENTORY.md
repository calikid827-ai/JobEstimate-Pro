...
- Shared EstimatorScopeFacts helper:
  ...
  - Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, backend Estimate Defense, backend missedScopeDetector, route-level display-only Scope-to-Price X-Ray / area confirmation diagnostics, `buildMaterialsList` confirmItems/notes, selected `materialsList.items` conditional gates, customer-facing trade coordination append gating, and schedule/rationale multi-trade text gating now consume EstimatorScopeFacts where safe. Remaining prompt-adjacent or diagnostic text paths that still use raw scope, `tradeStack`, or `complexityProfile`, plus broader route diagnostics, `scopeSplitter`, and pricing prep are not fully migrated/audited yet.
...
- The UI-side estimator stack plus backend Estimate Defense, missedScopeDetector, route-level display diagnostics, materials diagnostics, selected materials item gates, customer-facing coordination append gating, and schedule/rationale text gating now have EstimatorScopeFacts coverage across typed-scope normalization, Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, display-only Estimate Defense diagnostics, warning-only missed-scope diagnostics, display-only X-Ray / area confirmation diagnostics, materials confirmItems/notes, selected customer-visible item gates, coordination text gating, and schedule/rationale multi-trade text gating where safe.
- Remaining prompt-adjacent or diagnostic text paths that still use raw scope, `tradeStack`, or `complexityProfile`, plus broader route diagnostics, `scopeSplitter`, and pricing prep are not fully migrated/audited to EstimatorScopeFacts yet. Customer-Facing Scope / `result.text` remains preserved and was not broadly rewritten.
...
- Next active smart-estimator task: Phase 8D follow-up audit of remaining prompt-adjacent or diagnostic text paths that still use raw scope, `tradeStack`, or `complexityProfile`. Prompts, `effectiveScopeChange`, `result.text`, and route/API shape should not change without a scoped review.
...
1. Audit remaining prompt-adjacent or diagnostic text paths that still use raw scope, `tradeStack`, or `complexityProfile` as the next Phase 8D follow-up, without changing prompts, `effectiveScopeChange`, `result.text`, or route/API shape until the raw-scope risk is mapped.
