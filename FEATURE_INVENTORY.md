...
- Shared EstimatorScopeFacts helper:
  ...
  - Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, backend Estimate Defense, backend missedScopeDetector, and route-level display-only Scope-to-Price X-Ray / area confirmation diagnostics now consume EstimatorScopeFacts where safe. `buildMaterialsList` confirmation items/notes, materials diagnostics, `scopeSplitter`, materials generation, and pricing prep are not fully migrated/audited yet.
...
- The UI-side estimator stack plus backend Estimate Defense, missedScopeDetector, and route-level display diagnostics now have EstimatorScopeFacts coverage across typed-scope normalization, Scope-to-Price Consistency Review, Customer Scope Drift, Schedule Sequencing Review, PriceGuard Review, display-only Estimate Defense diagnostics, warning-only missed-scope diagnostics, and display-only X-Ray / area confirmation diagnostics.
- `buildMaterialsList` confirmation items/notes, materials diagnostics, `materialsList.items` generation, `scopeSplitter`, and pricing prep are not fully migrated/audited to EstimatorScopeFacts yet. Customer-Facing Scope / `result.text` remains preserved and was not broadly rewritten.
...
- Next active smart-estimator task: Phase 8B audit of `buildMaterialsList` confirmation items/notes and materials diagnostics for remaining raw scope parsing. `materialsList.items` generation should remain untouched until a separate scoped task proves it is safe because it is closer to customer-visible generated output and can affect user expectations.
...
1. Audit `buildMaterialsList` confirmation items/notes and materials diagnostics as Phase 8B of the shared scope-understanding architecture, without changing `materialsList.items` generation.
