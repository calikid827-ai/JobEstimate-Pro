...
- Full no-code end-to-end regression QA passed after `b4b26d8 Fix job template scope apply` using `test12345@gmail.com`. QA verified the five command-center sections, collapsed Advanced Diagnostics, Proposal actions, Rate Card save/persist/apply, Job Templates save/persist/apply after refresh, visible typed scope textarea prefill, print hiding for `data-no-print` workflow controls, copy/PDF localStorage stability for history/jobs/invoices, and no console/page errors.
