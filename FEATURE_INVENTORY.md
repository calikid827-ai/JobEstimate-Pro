...
1. Polish or audit the next optional intelligence layer around typed scope as the required scope-control anchor: Smart Questions V2B server/API authority audit only, Smart Questions V2B client-only authority-status readback, company intelligence / saved-estimate learning, camera measuring prototype, voice-to-scope draft, plan-to-scope reconciliation, or mobile/browser visual QA. Keep Evidence Authority internal/debug-only unless normal API/saved/UI/PDF exposure is separately scoped.
