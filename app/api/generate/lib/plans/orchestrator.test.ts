...
test("selected-sheet analysis only uses user-selected pages", async () => {
  ...
  assert.equal(result.evidenceStrength?.level, "useful")
  assert.equal(result.evidenceStrength?.selectedPagesCount, 1)
...
test("unselected pdf pages do not contribute plan findings", async () => {
  ...
  assert.match(result.evidenceStrength?.summary || "", /confirmation|quantities|Hard quantities/i)
...
test("browser-derived, server-derived, and fallback upload modes preserve identical estimator-ready package behavior downstream", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      serverResult.planReadback?.groupedScopeReadback.map((item) => item.narration)
    )
    assert.deepEqual(
      browserResult.planReadback?.groupedScopeReadback.map((item) => item.narration),
