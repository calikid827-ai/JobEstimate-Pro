...
const NON_SCOPE_CONTEXT_PATTERN =
  /\b(protect(?:ing|ion)?|safeguard(?:ing)?|cover(?:ing)?|mask(?:ing)?|adjacent\s+finishes?|avoid(?:ing)?\s+interference|prevent(?:ing)?\s+interference|no\s+interference|without\s+interference|coordinate|coordinates|coordinating|coordination|independently|other\s+trades?|proceed\s+independently|allow\s+other\s+trades|work(?:ing)?\s+around|around\s+existing|existing\s+(?:flooring|floors?|baseboards?|trim|cabinetry|cabinets?|door\s+jambs?|closets?|transitions?|electrical\s+(?:wiring|components?)))\b/i

const FLOORING_CONTEXT_ONLY_PATTERN =
  /\b(protect(?:s|ed|ing|ion)?|safeguard(?:ing)?|cover(?:ed|ing)?|drop\s+cloths?|floor\s+protection|work(?:ing)?\s+around|around\s+existing|coordinate|coordination|no\s+interference|avoid(?:ing)?\s+interference)\b.{0,80}\b(flooring|floors?|lvp|laminate|hardwood|carpet|transitions?)\b|\b(flooring|floors?|lvp|laminate|hardwood|carpet|transitions?)\b.{0,80}\b(protect(?:s|ed|ing|ion)?|safeguard(?:ing)?|cover(?:ed|ing)?|drop\s+cloths?|floor\s+protection|work(?:ing)?\s+around|around\s+existing|coordinate|coordination|no\s+interference|avoid(?:ing)?\s+interference)\b/i
...
const ELECTRICAL_COVER_PLATE_COORDINATION_CONTEXT_PATTERN =
  /\belectrical\s+devices?\b.{0,80}\b(?:to\s+prevent|prevent(?:ing)?|prevent)\b.{0,80}\b(?:paint\s+)?overspray\s+or\s+damage\b|\belectrical\s+devices?\b.{0,80}\b(?:mask(?:ed|ing)?|protect(?:ed|ing)?)\b.{0,100}\b(?:prevent(?:ing)?|to\s+prevent)\b.{0,80}\b(?:paint\s+)?overspray\b|\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\s+is\s+necessary\b.{0,180}\bfacilitat(?:e|es|ed|ing|ion)\b.{0,160}\b(?:removal|remove)\b.{0,80}\b(?:reinstallation|reinstall(?:ed|ing)?)\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,180}\bwithout\s+causing\s+damage\s+or\s+(?:interfering\s+with|disrupting)\s+(?:existing\s+)?wiring\b|\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\b.{0,160}\bnecessary\b.{0,160}\bfacilitat(?:e|es|ed|ing|ion)\b.{0,140}\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+(?:removal\s*\/\s*reinstallation|handling)\b|\belectrical\s+trade\s+coordination\b.{0,120}\bnecessary\b.{0,160}\bfacilitat(?:e|es|ed|ing|ion)\b.{0,120}\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+handling\b|\bremoval\s+and\s+reinstallation\s+of\s+(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,160}\bwithout\s+causing\s+damage\s+or\s+(?:interfering\s+with|disrupting)\s+(?:existing\s+)?wiring\b|\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+handling\b.{0,120}\bwithout\s+causing\s+damage\s+or\s+(?:interfering\s+with|disrupting)\s+(?:existing\s+)?wiring\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,80}\bhandled\b.{0,120}\bwithout\s+(?:damaging|disrupting|damaging\s+or\s+disrupting)\s+(?:existing\s+)?wiring\b/i

const ELECTRICAL_COVER_PLATE_UNAFFECTED_CONTEXT_PATTERN =
  /\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\s+is\s+necessary\b.{0,180}\bsafely\s+remov(?:e|ed|ing)?\b.{0,80}\breinstall(?:ed|ing)?\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\b.{0,160}\bnecessary\b.{0,120}\bsafely\s+remov(?:e|ed|ing)?\s*\/\s*reinstall(?:ed|ing)?\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\belectrical\s+trade\s+coordination\b.{0,120}\bnecessary\b.{0,160}\bsafely\s+remov(?:e|ed|ing)?\b.{0,80}\breinstall(?:ed|ing)?\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\bsafely\s+remov(?:e|ed|ing)?\b.{0,80}\breinstall(?:ed|ing)?\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,80}\bsafely\s+remov(?:e|ed|ing)?\b.{0,80}\breinstall(?:ed|ing)?\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,80}\bremov(?:e|ed|al)?(?:\s*\/\s*|\s+and\s+)?reinstall(?:ed|ation|ing)?\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+removal\s*\/\s*reinstallation\b.{0,140}\bwithout\s+affecting\s+(?:wiring|devices?)\b|\b(wiring|devices?)\b.{0,80}\b(?:are\s+not\s+affected|remain\s+unaffected)\b|\bwiring\s*\/\s*devices?\s+remain\s+unaffected\b/i
