...
const NON_SCOPE_CONTEXT_PATTERN =
  /\b(protect(?:ing|ion)?|safeguard(?:ing)?|cover(?:ing)?|mask(?:ing)?|adjacent\s+finishes?|avoid(?:ing)?\s+interference|prevent(?:ing)?\s+interference|no\s+interference|without\s+interference|coordinate|coordinates|coordinating|coordination|work(?:ing)?\s+around|around\s+existing|existing\s+(?:flooring|floors?|baseboards?|trim|cabinetry|cabinets?|door\s+jambs?|closets?|transitions?|electrical\s+(?:wiring|components?)))\b/i
...
const ELECTRICAL_CONTEXT_ONLY_PATTERN =
  /\b(no\s+interference|avoid(?:ing)?\s+interference|prevent(?:ing)?\s+interference|without\s+interference|coordinate|coordinates|coordinating|coordination|protect(?:ing|ion)?|safeguard(?:ing)?|independently|by\s+others|other\s+trades?)\b.{0,100}\b(electrical|electrician|wiring|components?|outlets?|switches?|lighting|fixtures?|trades?)\b|\b(electrical|electrician|wiring|components?|outlets?|switches?|lighting|fixtures?|trades?)\b.{0,100}\b(no\s+interference|avoid(?:ing)?\s+interference|prevent(?:ing)?\s+interference|without\s+interference|coordinate|coordinates|coordinating|coordination|protect(?:ing|ion)?|safeguard(?:ing)?|independently|by\s+others|other\s+trades?)\b|\b(existing|adjacent)\s+electrical\s+(wiring|components?)\b/i
...
const PLUMBING_CONTEXT_ONLY_PATTERN =
  /\b(no\s+interference|avoid(?:ing)?\s+interference|without\s+interference|coordinate|coordinates|coordinating|coordination|independently|by\s+others|other\s+trades?|allow\s+other\s+trades)\b.{0,100}\b(plumbing|plumber|water\s+lines?|supply\s+lines?|drains?|valves?|fixtures?|trades?)\b|\b(plumbing|plumber|water\s+lines?|supply\s+lines?|drains?|valves?|fixtures?|trades?)\b.{0,100}\b(no\s+interference|avoid(?:ing)?\s+interference|without\s+interference|coordinate|coordinates|coordinating|coordination|independently|by\s+others|other\s+trades?|allow\s+other\s+trades)\b/i
