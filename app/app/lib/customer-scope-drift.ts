...
const ELECTRICAL_COVER_PLATE_LIMITED_HANDLING_CONTEXT_PATTERN =
  /\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\b.{0,120}\b(?:is\s+)?(?:limited\s+to|for)\b.{0,100}\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+handling\s+only\b|\belectrical\s+trade\s+coordination\b.{0,120}\blimited\s+to\b.{0,120}\bremov(?:ing|e|al)?\b.{0,80}\breinstall(?:ing|ation|ed)?\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,100}\bpainting\s+only\b|\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+handling\s+only\b.{0,100}\bno\s+electrical\s+work\b/i
...
const DRYWALL_SUBSTRATE_CONTEXT_ONLY_PATTERN =
  /\b(?:standard|existing|paintable|previously\s+painted)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|substrates?)\b.{0,120}\b(?:paint|painting|painted|receive\s+paint|coats?)\b|\b(?:paint|painting|painted|coats?)\b.{0,120}\b(?:over\s+)?(?:standard|existing|paintable|previously\s+painted)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|substrates?)\b|\bassum(?:e|es|ed|ing)\b.{0,80}\bstandard\s+(?:drywall|sheetrock|gypsum)\s+surfaces?\b/i
