...
const DRYWALL_PAINT_PREP_CONTEXT_ONLY_PATTERN =
  /\b(?:light|minor)\s+(?:sanding\s+and\s+)?(?:patching|dust\s+removal)\b.{0,140}\b(?:standard|existing|paintable|previously\s+painted|prepared)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b|\b(?:light|minor)\s+(?:sanding\s+and\s+)?patching\s+of\s+(?:minor\s+)?(?:wall\s+)?imperfections\b.{0,120}\b(?:standard|existing|paintable|previously\s+painted)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b|\b(?:standard|existing|paintable|previously\s+painted|prepared)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b.{0,120}\b(?:light|minor)\s+(?:sanding\s+and\s+)?(?:patching|dust\s+removal)\b|\b(?:primer|finish\s+coats?|paint\s+materials?)\b.{0,120}\bappropriate\s+for\s+(?:drywall|sheetrock|gypsum)\b|\bexisting\s+(?:drywall|sheetrock|gypsum)\b.{0,120}\b(?:good|sound)\s+condition\b.{0,120}\b(?:without\s+requiring\s+extensive\s+repairs?|free\s+of\s+significant\s+defects\s+requiring\s+repair)\b/i
