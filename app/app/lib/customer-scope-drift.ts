...
const TRADE_RULES: TradeRule[] = [
  ...
  },
  {
    ...
    mentionPattern:
      /\b(carpentry|carpenter|framing|blocking|baseboards?|casing|crown|trim\s+install|(?:install|replace|repair)\s+trim|door\s+(install|replacement)|shelving|millwork)\b/i,
    supportPattern:
      /\b(carpentry|carpenter|framing|blocking|baseboards?|casing|crown|trim\s+install|(?:install|replace|repair)\s+trim|door\s+(install|replacement)|shelving|millwork|\b\d+(\.\d+)?\s?(lf|linear\s+feet|linear\s+foot)\b)\b/i,
  },
  {
    id: "wallcovering",
    label: "wallcovering",
...
const FLOORING_BASEBOARD_FITMENT_CONTEXT_PATTERN =
  /\bflooring\s+installation\b.{0,220}\b(?:transition\s+strips?|transitions?|layout|cutting|fitting|securing|subfloor)\b.{0,220}\b(?:reattachment|reinstall(?:ation|ing)?|replacement)(?:\s+or\s+(?:reattachment|reinstall(?:ation|ing)?|replacement))?\s+of\s+baseboards?\s+as\s+needed\b|\b(?:transition\s+strips?|transitions?)\b.{0,180}\b(?:reattachment|reinstall(?:ation|ing)?|replacement)(?:\s+or\s+(?:reattachment|reinstall(?:ation|ing)?|replacement))?\s+of\s+baseboards?\s+as\s+needed\b/i
