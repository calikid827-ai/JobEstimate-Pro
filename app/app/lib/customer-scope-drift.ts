...
const TRADE_RULES: TradeRule[] = [
  ...
  },
  {
    ...
    mentionPattern:
      /\b(carpentry|carpenter|framing|blocking|baseboards?|thresholds?|casing|crown|trim\s+install|(?:install|replace|repair)\s+trim|door\s+(install|replacement)|shelving|millwork)\b/i,
    supportPattern:
      /\b(carpentry|carpenter|framing|blocking|baseboards?|thresholds?|casing|crown|trim\s+install|(?:install|replace|repair)\s+trim|door\s+(install|replacement)|shelving|millwork|\b\d+(\.\d+)?\s?(lf|linear\s+feet|linear\s+foot)\b)\b/i,
  },
  {
    id: "wallcovering",
    label: "wallcovering",
...
const ELECTRICAL_COVER_PLATE_GENERATED_CONTEXT_PATTERN =
  /\belectrical\s+(?:devices?|components?)\b.{0,80}\b(mask(?:ed|ing)?|protect(?:ed|ing|ion)?)\b|\b(mask(?:ed|ing)?|protect(?:ed|ing|ion)?)\b.{0,80}\belectrical\s+(?:devices?|components?)\b|\belectrical\s+outlet\s+covers?\b|\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\b.{0,120}\b(confined|limited|strictly\s+limited|required\s+to\s+manage|manage)\b.{0,140}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\belectrical\s+trade\s+coordination\b.{0,120}\bmanage\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\belectrical\s+coordination\b.{0,100}\b(confined|limited|strictly\s+limited)\b.{0,120}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\bmanage\s+(?:the\s+)?(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,140}\bwithout\s+causing\s+damage\s+to\s+(?:wiring|devices?)\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,80}\bremov(?:e|ed|al)?(?:\s*\/\s*|\s+and\s+)?reinstall(?:ed|ation|ing)?\b.{0,140}\bwithout\s+(?:disturbing|electrical\s+work|rewiring|device\s+replacement)\b|\bremov(?:e|ed|al)?(?:\s*\/\s*|\s+and\s+)?reinstall(?:ed|ation|ing)?\b.{0,80}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,140}\bwithout\s+(?:disturbing|electrical\s+work|rewiring|device\s+replacement)\b|\bwithout\s+causing\s+damage\s+to\s+(?:wiring|devices?)\b|\bno\s+damage\s+to\s+(?:wiring|devices?)\b|\b(?:wiring|devices?)\b.{0,80}\b(?:to\s+remain\s+untouched|remain\s+(?:existing|to\s+remain))\b|\bno\s+(?:electrical\s+)?rewiring\b|\bno\s+(?:electrical\s+)?(?:device\s+)?replacement\b|\bno\s+electrical\s+work\s+beyond\b.{0,120}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\bno\s+modifications?\s+to\s+the\s+electrical\s+system\b/i
...
const ELECTRICAL_COVER_PLATE_LIMITED_HANDLING_CONTEXT_PATTERN =
  /\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\b.{0,120}\b(?:is\s+)?(?:limited\s+to|strictly\s+limited\s+to|for|required\s+for)\b.{0,100}\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+handling\s+only\b|\bcoordination\s+with\s+(?:the\s+)?electrical\s+trade\b.{0,140}\bstrictly\s+limited\s+to\b.{0,100}\b(?:remov(?:ing|e|al)?|removal)\b.{0,80}\b(?:reinstall(?:ing|ation|ed)?|reinstallation)\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\bcoordination\s+for\s+electrical\s+access\b.{0,120}\brestricted\s+to\b.{0,100}\bremoval\s+of\s+(?:outlet\s+covers?|switch\s+covers?|cover\s+plates?)\s+only\b|\belectrical\s+trade\s+coordination\b.{0,120}\b(?:is\s+)?limited\s+to\b.{0,120}\b(?:remov(?:ing|e|al)?|removal)\b.{0,80}\b(?:reinstall(?:ing|ation|ed)?|reinstallation)\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,100}\bpainting\s+only\b|\belectrical\s+coordination\s+only\b.{0,120}\b(?:remov(?:ing|e|al)?|removal)\b.{0,80}\b(?:reinstall(?:ing|ation|ed)?|reinstallation)\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,100}\bpainting\b|\b(outlet\s+cover|switch\s+cover|cover\s+plate)s?\s+handling\s+only\b.{0,120}\bno\s+(?:electrical\s+work|wiring|device|circuit)\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,80}\bhandled\s+only\b.{0,120}\bno\s+(?:electrical\s+work|wiring|device|circuit)\b|\b(?:replacement|replacements?|modification|modifications?|repairs?)\s+of\s+electrical\s+devices?\b.{0,120}\bbeyond\s+(?:the\s+)?handling\s+of\s+(?:outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\bno\s+electrical\s+(?:repairs?|modifications?|repairs?\s+or\s+modifications?)\b.{0,120}\bbeyond\b.{0,80}\bhandl(?:e|ed|ing)\b.{0,80}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b/i

const ELECTRICAL_COVER_PLATE_SAFE_HANDLING_CONTEXT_PATTERN =
  /\b(?:coordination\s+with\s+(?:the\s+)?electrical\s+trade|electrical\s+trade\s+coordination)\b.{0,220}\b(?:safely\s+)?(?:remov(?:e|al|ing)|reinstall(?:ation|ing)?|handl(?:e|ed|ing))\b.{0,180}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,220}\bwithout\s+(?:causing\s+damage\s+to|disrupting|affecting|interfering\s+with|disturbing)\s+(?:existing\s+)?(?:wiring|devices?|circuits?)(?:\s+or\s+(?:wiring|devices?|circuits?))?\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,180}\b(?:safely\s+)?(?:remov(?:e|ed|al|ing)|reinstall(?:ed|ation|ing)?|handl(?:e|ed|ing))\b.{0,220}\bwithout\s+(?:causing\s+damage\s+to|disrupting|affecting|interfering\s+with|disturbing)\s+(?:existing\s+)?(?:wiring|devices?|circuits?)(?:\s+or\s+(?:wiring|devices?|circuits?))?\b|\b(?:coordinat(?:e|ed|ion))\b.{0,160}\bavoid\s+interference\s+with\s+(?:existing\s+)?wiring\s+and\s+devices?\b.{0,120}\bduring\b.{0,80}\b(?:removal\s*\/\s*reinstallation|removal|reinstallation|remov(?:ing|al)?|reinstall(?:ing|ation)?)\b|\b(?:removal|remov(?:e|ed|ing))\b.{0,100}\b(?:storage|careful\s+storage)\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,180}\b(?:prior\s+to\s+(?:the\s+)?commencement\s+of\s+painting|before\s+painting|for\s+painting|paint(?:ing)?)\b|\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,100}\b(?:removal|remov(?:e|ed|ing))\b.{0,140}\b(?:storage|careful\s+storage)\b.{0,180}\b(?:prior\s+to\s+(?:the\s+)?commencement\s+of\s+painting|before\s+painting|for\s+painting|paint(?:ing)?)\b|\b(?:upon\s+completion\s+of\s+painting|following\s+the\s+painting\s+process|after\s+painting|after\s+the\s+outlet\s+covers?\s+are\s+removed)\b.{0,160}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,80}\b(?:reinstalled|removed)\b|\b(?:paint|painting|coats?)\b.{0,160}\bafter\s+the\s+(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\s+are\s+removed\b|\b(?:meticulous\s+)?removal\b.{0,80}\bsubsequent\s+reinstallation\b.{0,100}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b.{0,160}\b(?:facilitate|painting|wall\s+painting)\b|\bwithout\s+modifying\s+electrical\s+components?\b.{0,120}\bbeyond\s+(?:the\s+)?handling\s+of\s+(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b|\bno\s+electrical\s+work\s+beyond\b.{0,120}\bhandl(?:e|ed|ing)\b.{0,80}\b(outlet\s+covers?|switch\s+covers?|cover\s+plates?)\b/i
...
const DRYWALL_SUBSTRATE_CONTEXT_ONLY_PATTERN =
  /\b(?:standard|existing|paintable|previously\s+painted|existing\s+standard|standard\s+existing)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b.{0,120}\b(?:paint|painting|painted|receive\s+paint|coats?|wall\s+painting)\b|\b(?:paint|painting|painted|coats?|wall\s+painting|targeting)\b.{0,140}\b(?:over\s+)?(?:standard|existing|paintable|previously\s+painted|existing\s+standard|standard\s+existing)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b|\bassum(?:e|es|ed|ing)\b.{0,80}\bstandard\s+(?:drywall|sheetrock|gypsum)\s+surfaces?\b|\b(?:drywall|sheetrock|gypsum)\b.{0,80}\b(?:and|\/)\s*paint(?:ing)?\b|\bpaint(?:ing)?\b.{0,80}\b(?:and|\/)\s*(?:drywall|sheetrock|gypsum)\b/i

const DRYWALL_PAINT_PREP_CONTEXT_ONLY_PATTERN =
  /\b(?:light|minor)\s+(?:sanding\s+and\s+)?(?:patching|dust\s+removal)\b.{0,140}\b(?:standard|existing|paintable|previously\s+painted|prepared|existing\s+standard|standard\s+existing)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b|\b(?:light\s+sanding\s+and\s+)?patching\s+(?:of\s+)?minor\s+(?:drywall|sheetrock|gypsum)\s+imperfections\b.{0,120}\b(?:paint\s+adhesion|painting|paint|coats?)\b|\b(?:light|minor)\s+(?:sanding\s+and\s+)?patching\s+of\s+(?:minor\s+)?(?:wall\s+)?imperfections\b.{0,120}\b(?:standard|existing|paintable|previously\s+painted|existing\s+standard|standard\s+existing)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b|\b(?:standard|existing|paintable|previously\s+painted|prepared|existing\s+standard|standard\s+existing)?\s*(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?)\b.{0,120}\b(?:light|minor)\s+(?:sanding\s+and\s+)?(?:patching|dust\s+removal)\b|\b(?:primer|primer\s*\/\s*sealer|finish\s+paint|finish\s+coats?|paint\s+materials?|standard\s+interior\s+wall\s+paint|interior\s+latex\s+paint)\b.{0,160}\b(?:appropriate\s+for|suitable\s+for|on|coverage\s+of)\s+(?:the\s+)?(?:standard\s+)?(?:interior\s+)?(?:drywall|sheetrock|gypsum)(?:\s+surfaces?)?\b|\b(?:concurrent|ongoing\s+)?(?:drywall|sheetrock|gypsum)(?:\s+or\s+)?(?:renovation\s+activities|finishing\s+work)\b.{0,140}\b(?:minimi[sz]e\s+disruption|prevent\s+conflicts|coordination|coordinat(?:e|ed|ion))\b|\b(?:minimi[sz]e\s+disruption|prevent\s+conflicts|coordination|coordinat(?:e|ed|ion))\b.{0,140}\b(?:concurrent|ongoing\s+)?(?:drywall|sheetrock|gypsum)(?:\s+or\s+)?(?:renovation\s+activities|finishing\s+work)\b|\b(?:assumes?|existing|standard)\b.{0,100}\b(?:drywall|sheetrock|gypsum)\s+(?:surfaces?|walls?|wall\s+surfaces?|substrates?|walls?\s+and\s+ceilings?)\b.{0,180}\b(?:without|no|not|requiring\s+no)\b.{0,120}\b(?:concealed\s+damage\s+requiring\s+corrective\s+work|corrective\s+work|(?:extensive\s+)?demolition|(?:extensive\s+)?repairs?|repair\s+work|replacement|major\s+repairs?)\b|\bexisting\s+(?:drywall|sheetrock|gypsum)(?:\s+surfaces?)?\b.{0,120}\b(?:good|sound|satisfactory|generally\s+in\s+sound)\s+condition\b.{0,160}\b(?:without\s+requiring\s+extensive\s+repairs?|with\s+no\s+requirement\s+for\s+patching\s+or\s+repair|free\s+of\s+significant\s+defects\s+requiring\s+repair|does\s+not\s+include\s+(?:drywall|sheetrock|gypsum)\s+replacement\s+or\s+major\s+repairs?)?\b/i
...
const FLOORING_BASEBOARD_FITMENT_CONTEXT_PATTERN =
  /\bflooring\s+installation\b.{0,220}\b(?:transition\s+strips?|transitions?|thresholds?|layout|cutting|fitting|fitment|securing|subfloor|adjacent\s+surfaces?|trimming)\b.{0,220}\b(?:reattachment|reinstall(?:ation|ing)?|replacement)(?:\s+or\s+(?:reattachment|reinstall(?:ation|ing)?|replacement))?\s+of\s+baseboards?\s+as\s+needed\b|\b(?:transition\s+strips?|transitions?)\b.{0,180}\b(?:reattachment|reinstall(?:ation|ing)?|replacement)(?:\s+or\s+(?:reattachment|reinstall(?:ation|ing)?|replacement))?\s+of\s+baseboards?\s+as\s+needed\b|\bflooring\s+installation\b.{0,240}\b(?:thresholds?|transitions?|cutting|fitting|fitment|securing|trimming|adjacent\s+surfaces?)\b.{0,200}\b(?:thresholds?|transitions?|cutting|fitting|fitment|securing|trimming|adjacent\s+surfaces?)\b/i

const CARPENTRY_TRUE_WORK_PATTERN =
  /\b(baseboards?|thresholds?|casing|crown|trim|doors?|shelving|millwork|cabinetry|cabinets?)\s+(replacement|replace|install(?:ation|ing)?|repair(?:ing|s)?)\b|\b(replace(?:ment|ing)?|install(?:ation|ing)?|repair(?:ing|s)?)\s+(?:\w+\s+){0,2}(baseboards?|thresholds?|casing|crown|trim|doors?|shelving|millwork|cabinetry|cabinets?)\b|\b(framing|blocking)\b/i
...
function isNonScopeContextMention(part: string, rule: TradeRule) {
  ...
  if (rule.id === "carpentry" && FLOORING_BASEBOARD_FITMENT_CONTEXT_PATTERN.test(part)) {
    return true
  }
...
function tradeExclusionConflict(rule: TradeRule, args: BuildCustomerScopeTradeDriftWarningArgs) {
  ...
  if (rule.id === "electrical") {
    return sentenceParts(args.resultText).some(
      (part) =>
        ELECTRICAL_TRUE_WORK_PATTERN.test(part) &&
        !EXCLUDED_TRADE_PATTERN.test(part) &&
        !isNonScopeContextMention(part, rule)
    )
  }
