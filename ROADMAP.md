...
2. Next active product-improvement task: Phase 8D follow-up audit of remaining prompt-adjacent or diagnostic text paths that still use raw scope, `tradeStack`, or `complexityProfile`. Keep prompts, `effectiveScopeChange`, `result.text`, and route/API shape unchanged unless a later scoped task explicitly changes them.
...
- NEXT: Phase 8D follow-up audit remaining prompt-adjacent or diagnostic text paths that still use raw scope, `tradeStack`, or `complexityProfile`. Audit this next because those paths may still affect schedule/story/diagnostic language, but prompts and `result.text` must not change without a scoped review first.
