...
2. Next active product-improvement task: Phase 8D follow-up audit of remaining prompt-adjacent route logic where `tradeStack` or complexity can still influence schedule/rationale text. Keep prompts, `effectiveScopeChange`, `result.text`, and route/API shape unchanged unless a later scoped task explicitly changes them.
...
- NEXT: Phase 8D follow-up audit remaining prompt-adjacent route logic where `tradeStack` or complexity can still influence schedule/rationale text. Audit this next because those paths are adjacent to customer-facing schedule/story language, but prompts and `result.text` must not change without a scoped review first.
