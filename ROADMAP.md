...
2. Next active product-improvement task: Phase 8C audit of remaining route-level `materialsList.items` generation for raw scope parsing. Keep this audit/planning only first because item generation is customer-visible and higher risk than confirmation notes.
...
- NEXT: Phase 8C audit remaining route-level `materialsList.items` generation for raw scope parsing. Audit this next because item generation still has raw scope parsing in some branches, but it is customer-visible and higher risk, so it needs audit/planning before any implementation.
