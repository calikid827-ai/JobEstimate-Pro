...
2. Next active product-improvement task: Phase 8D follow-up audit of remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `buildEstimateExplanation`, `profitLeakDetector`, and photo-estimate decision text. Keep prompts, `effectiveScopeChange`, `result.text`, pricing policy, and route/API shape unchanged unless a later scoped task explicitly changes them.
...
- NEXT: Phase 8D follow-up audit remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `buildEstimateExplanation`, `profitLeakDetector`, and photo-estimate decision text. Audit this next because those paths may still affect schedule/story/diagnostic language, but prompts, `effectiveScopeChange`, `result.text`, pricing policy, and route/API shape must not change without a scoped review first.
