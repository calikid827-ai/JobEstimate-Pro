...
2. Next active product-improvement task: Phase 9B-display audit/planning for how to expose Evidence Authority readback safely. Do not implement broad UI/API/display wiring until response, saved estimate, and UI shape changes are explicitly scoped.
...
- NEXT: Phase 9B-display should start with an audit/planning pass for how to expose Evidence Authority readback safely. Exposing it would intentionally change response/saved/UI shape, so do not implement broad display wiring until that shape decision is explicitly scoped. Deferred photo behavior items remain pricing/policy-adjacent: polluted multi-trade signals can still affect confidence penalty and measurement-heavy behavior, and raw owner-supplied / by-others quantity parsing remains unchanged.
