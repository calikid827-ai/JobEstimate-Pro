...
- NEXT: Phase 9B-estimator-display should be a separately scoped audit/planning or implementation decision for exposing Evidence Authority beyond debug. Do not expose Evidence Authority in normal API responses, saved estimates, UI, or PDFs/customer output until response/saved/UI shape changes are explicitly scoped. Deferred photo behavior items remain pricing/policy-adjacent: polluted multi-trade signals can still affect confidence penalty and measurement-heavy behavior, and raw owner-supplied / by-others quantity parsing remains unchanged.
