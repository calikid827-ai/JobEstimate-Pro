...
2. DONE: Actuals vs Estimate Feedback V1 is complete in `6f535d2` and `910391d`. It remains reference-only and does not automatically change Rate Card defaults, production rates, pricing formulas, pricing authority, labor totals, or future estimates.
3. DONE: The bounded Whole-Product Intelligence & Simplification Review is complete. Its one pre-Photo implementation, Canonical Contractor Status Semantics Cleanup, is complete in `2c8ebb1`: Send Readiness remains the sole send/no-send verdict, Estimator Review uses panel-scoped findings language, and paid invoice workflow displays payment status without implying job completion.
4. DONE: Photo Intelligence Actions V1 is complete across evidence-bound action model V1A (`2e7be1a`) and runtime integration V1B (`9832108`).
5. NEXT PRODUCT-INTELLIGENCE FEATURE: Plan Intelligence Workflow V1.
6. Follow with Contractor Learning System after the reference-only feedback and evidence workflows are stable.
7. Continue using `PRODUCT_AUDIT_BACKLOG.md` as the master improvement tracker and work each phase as a focused change. Typed scope remains the required scope-control anchor, and plan/photo/crew/Smart Question signals remain non-pricing-authoritative unless a later task explicitly scopes authority and verification.
8. Keep final Production Live Mode subscription verification using `SUBSCRIPTION_TEST_CHECKLIST.md` pending as the final pre-launch gate before accepting public paid users.
...
- DONE: UI-only Estimator Review noise reduction is complete. Estimator Review Summary is the primary review hub, Customer Output Readiness sits below the summary and the current Scope Decisions interaction as compact/collapsed details that auto-open for critical unsupported trade/scope drift, PriceGuard Review and Estimator Diagnostics remain available but collapsed, and nested diagnostics such as Scope-to-Price X-Ray, Materials List, Area Scope Breakdown, and Profit Protection are collapsed by default. This reduced duplicate review messaging without changing backend/API/saved/PDF/pricing/prompt/result/materials/deterministic/billing behavior; Smart Questions remain local-only and non-pricing-authoritative, and Evidence Authority remains internal/debug-only.
...
- DONE: Contractor-facing workflow upgrade checkpoint is complete across commits `2f7fffe`, `5d458bf`, `b926fa6`, `0b503aa`, `b4b26d8`, `2261e10`, and `489351c`. Generated results use five primary Command Center sections: Proposal, Price & Profit, Schedule & Crew, Review Before Sending, and Job Workflow, with Advanced Diagnostics collapsed inside Review Before Sending and EstimateStatusCard inside that drawer. Saved Job Templates V1 is client-only in `jobestimatepro_templates_v1`, stores safe setup fields only, and applying templates prefills estimator inputs without auto-generating; the visible typed scope textarea prefill after refresh is fixed. Rate Card V1 is client-only in `jobestimatepro_rate_card_v1`, stores safe local markup/tax/deposit defaults plus reference-only notes, and applying it updates only editable pricing controls without becoming backend pricing authority. Proposal has compact delivery actions for existing PDF download behavior and copying customer-facing `result.text`; controls are `data-no-print` and PDF content was not changed. Field Handoff V1 lives inside Job Workflow, derives crew-ready handoff notes from existing estimate state only, excludes diagnostics/internal review and PriceGuard content, adds no localStorage key or server persistence, and does not change pricing authority, PDF content, or `/api/generate`. Job Workflow Dashboard Polish V1 added a compact Active Job Summary at the top of the existing Job Workflow section, keeps Jobs, Job Templates, Invoices, Saved Estimates, and Field Handoff below it, derives job/client/status/contract/approval/invoice/recorded-cost workflow/crew/next-action information from existing state only, scopes current-estimate/Field Handoff readiness to the summarized job, and reuses existing nearby workflow actions or scrolls to existing workflow surfaces. It did not add storage, server persistence, new saved data shapes, pricing authority, PDF content, approval/invoice behavior, billing/auth changes, or `/api/generate` changes. Full no-code end-to-end regression QA passed after `489351c` using `test12345@gmail.com`.
- DONE: Proposal Readiness Badge V1 is complete through `37c8a14` and severity correction `0afc82e`. The existing Proposal section now places a compact `data-no-print` Send Readiness summary above the unchanged PDF/copy actions without adding a section or diagnostic panel. It reduces interpretation overhead by selecting one status and highest-priority reason from existing scope-drift, Customer Output Readiness, Estimator Review, PriceGuard, high-priority Smart Question, and plan/photo signals; optional Review items scrolls only to Review Before Sending. `Ready to send`, `Review before sending`, and the unused-in-current-UI pre-result fallback `Finish estimate first` are helper states. The severity follow-up keeps Unsupported trade wording critical but treats Customer-ready review as an ordinary PriceGuard review row. It changed no thresholds, item generation, helper priority, pricing/PDF/API/saved/storage/approval/invoice/workflow behavior, or server-backed logic. Full regression QA after `37c8a14` and focused QA after `0afc82e` passed with `test12345@gmail.com`; browser QA did not naturally observe Ready, while focused helper coverage and a complete code-level painting case confirm the clean state remains reachable.
- DONE: Actionable Scope Decisions V1 is complete in `7c5ac91`. Scope Decisions replaces Quick Clarifications in the existing Review Before Sending position and renders zero to three safely structured decisions without adding a section or diagnostics surface. It previews deterministic wording before explicit Apply, changes only typed scope, marks the displayed proposal `Regenerate before sending`, freezes generated-result readbacks until successful manual regeneration, counts actionable and review-only high-priority Smart Questions in readiness, and fails closed after contractor edits so feature-owned replacement cannot rewrite contractor prose. Runtime actionability remains allowlist-only: structured material responsibility plus the verified-current painting photo subjects documented under Photo Intelligence Actions V1; untrusted quantity, generic plan/photo/AI prose, and ambiguous subject inference remain deferred. No Generate/API payload, pricing/labor authority, PDF, saved shape, localStorage, approval/invoice, workflow, billing/auth, or server persistence behavior changed. Post-push validation passed TypeScript, 54/54 focused tests, and browser regression with `test12345@gmail.com`.
- DONE: Actuals vs Estimate Feedback V1 is complete across helper V1A (`6f535d2`) and Job Workflow UI V1B (`910391d`). It anchors only to the exact nonblank original saved estimate, compares Labor, Materials, and Subs / other with contractor-recorded costs to date, and leaves missing values as Not recorded. The compact active-job surface is reference-only, hidden from print, preserves exactly five Command Center sections, and does not calculate final actual cost, profit, margin, completion, invoice/billing variance, or recommendations. Existing actual-cost persistence remains unchanged; edits do not call Generate or mutate Rate Card, pricing, proposals/PDFs, history, or invoices. Validation passed TypeScript, 40/40 focused tests, focused review, and browser regression with sequential input-focus, mobile, print, state-preservation, and runtime-error checks.
- DONE: Phase 10A Estimator Intelligence Findings backbone is complete. Phase 10A-1 added the client-safe pure mapper in `app/app/lib/estimator-intelligence-findings.ts` plus focused tests in `app/app/lib/estimator-intelligence-findings.test.ts`; validation passed with `npm run test:estimator -- app/app/lib/estimator-intelligence-findings.test.ts` 16/16, `npx tsc --noEmit`, and `git diff --check`. Phase 10A-2 added the estimator-only `data-no-print` `EstimatorIntelligenceFindingsPanel` and smallest wiring in `app/app/page.tsx`; Playwright Chromium desktop QA passed with screenshots at `/tmp/jobestimatepro-phase-10a-2-intelligence-findings-panel-qa`. The panel appears only after estimate generation when findings exist, lives inside the Estimator Review/internal review area, is collapsed by default, shows the guardrail “Review-only. Not included in pricing unless estimator confirms.”, is hidden by print media, and did not alter customer-facing scope, pricing text, estimate rows, Crew Planning, Smart Questions, or regeneration behavior. No route/orchestrator/API response shape, saved estimate/history shape, pricing totals, prompts, PDFs/customer output, billing/auth/deployment behavior changed.
...
- DONE: Crew Planning multi-trade clarity polish is complete in commit `Clarify multi-trade crew planning`. Crew Planning now adds `planningNotes` and shows a compact estimator-only note inside the `data-no-print` Crew Planning panel when typed scope clearly includes multiple trades, such as “Paint 3 bedrooms and install flooring” or “Paint 3 bedrooms and replace outlets.” Protection-only floor wording such as protect floors with drop cloths, floor protection, flooring protected with drop cloths, and drop cloths over flooring does not trigger the note, and General Renovation with painting-heavy typed scope still uses the painting sequence when there is no real second trade. Crew Planning remains client-only, estimator-only, not pricing-authoritative, not sent to `/api/generate`, not saved, not included in PDFs/customer output, and `affectsPricing: false`. No backend/API/saved/PDF/pricing/prompt/`result.text`/materials/deterministic/billing behavior changed; Smart Questions remain local-only and non-pricing-authoritative, and Evidence Authority remains internal/debug-only.
- DONE: Crew Planning V2 daily work-plan is complete in commit `Add crew planning daily work plan`. Crew Planning now includes `dailyPlan`, `dailyPlanNotes`, and `dailyPlanConfidence`; each Daily Work Plan item includes `label`, `crewSize`, `tasks`, `reminders`, `risks`, and `guidanceOnly: true`. Daily Work Plan renders inside the existing Crew Planning panel only, below crew options and above Work Sequence; no new top-level panel was added. Simple painting, dry-time painting, flooring protection, true painting/flooring, painting/electrical, and hotel/multi-unit scenarios passed final desktop/mobile Playwright QA with `test12345@gmail.com`, with artifacts at `/tmp/jobestimatepro-crew-planning-v2-daily-plan-qa/final-run-5`. Hotel/multi-unit uses rolling production entries rather than room-level cards, true flooring/electrical work still surfaces clearly, and Customer Output Readiness stays collapsed for context-only protection/masking. Optional later polish: simple painting may show Visit 1 / Visit 2 when generated schedule reports 2 visits even without explicit return-trip wording; readable but slightly less calm than a single Day 1. Crew Planning remains client-only, estimator-only, `data-no-print`, not pricing-authoritative, not sent to `/api/generate`, not saved, not included in PDFs/customer output, and `affectsPricing: false`. No backend/API/saved/PDF/pricing/prompt/`result.text`/materials/deterministic/billing behavior changed; Smart Questions remain local-only and non-pricing-authoritative, and Evidence Authority remains internal/debug-only.
- DONE: Customer Scope Drift electrical masking polish is complete in commit `Suppress electrical masking drift warnings`. Painting-context outlet/switch masking, cover-plate, and fixtures-to-remain wording no longer creates unsupported electrical drift or auto-opens Customer Output Readiness, while true unsupported electrical work still warns for replace/install outlets, move switches, repair wiring, electrical rough-in, run new wire, add circuit, panel work, and install light fixtures. This was UI/client review-only and did not change backend/API/saved/PDF/pricing/prompt/`result.text`/materials/deterministic/billing behavior; Smart Questions remain local-only and non-pricing-authoritative, and Evidence Authority remains internal/debug-only.
- DONE: Customer Scope Drift / Estimator Review false-positive cleanup is complete through commit `Suppress final generated context drift`. Customer Scope Drift now treats drywall substrate/prep/material/condition wording, flooring protection and flooring threshold/transition/fitment context, and outlet-cover/cover-plate handling-only wording as context-only while preserving true trade warnings for drywall, electrical/circuit, flooring, carpentry, baseboard, and threshold work. Customer Output Readiness stays collapsed for context-only cases, Estimator Review Summary remains compact, and Crew Planning avoids false multi-trade notes for drywall substrate / painting-prep and cover-plate-only prep while preserving true multi-trade/boundary planning behavior. Final desktop+iPhone Playwright screenshot QA passed with `test12345@gmail.com`, Chromium 1.60.0, 18/18 scenarios completed, 0 runner failures, and artifacts in `/tmp/jobestimatepro-final-generated-context-drift-qa/final-run-1`. This was UI/client review-only and did not change backend/API/saved/PDF/pricing/prompt/`result.text`/materials/deterministic/billing behavior; Smart Questions remain local-only and non-pricing-authoritative, and Evidence Authority remains internal/debug-only.
...
- DONE: Smart Questions V1 polish established the former `Quick Clarifications` interaction after `EstimatorReviewSummaryPanel`; Actionable Scope Decisions V1 now replaces it in the same Review Before Sending position. The Smart Question layer remains client-side/local-only and non-pricing-authoritative, is not saved as decision state or included in PDFs/customer output, and does not expose Evidence Authority in normal UI.
...
- DONE: Actuals vs Estimate Feedback V1 is reference-only and complete. The bounded Whole-Product Intelligence & Simplification Review and its focused Canonical Contractor Status Semantics Cleanup (`2c8ebb1`) are also complete.
- DONE: Photo Intelligence Actions V1 is complete across V1A (`2e7be1a`) and V1B (`9832108`). Exact ordered Generate-request photo evidence is fingerprinted in session, and only verified-current painting ceiling or trim/baseboard/casing flags can enter the existing Scope Decisions workflow inside Review Before Sending. Ordinary Scope Decisions retain boundary precedence and the existing three-decision cap. The former Proposal Smart Scope Assist Apply/direct-Regenerate controls are retired while photo analysis and readbacks remain. Evidence changes suppress prior actions and feed canonical Send Readiness as `Regenerate before sending`; legacy or loaded saved readbacks remain unverified and non-actionable until successful regeneration. Apply uses deterministic Scope Decisions wording, changes only typed scope, and still requires manual regeneration; photos do not directly change pricing, Rate Card, labor, production rates, proposal/PDF output, API/schema, or persistence. V1A passed TypeScript, 58/58 focused tests, and final review; V1B passed TypeScript, 81/81 focused tests after its narrow Change Order lifecycle fix, final verification, and deterministic browser QA with `test12345@gmail.com`.
- NEXT: Plan Intelligence Workflow V1 is the next product-intelligence feature, followed by Contractor Learning System as a separately scoped phase.
