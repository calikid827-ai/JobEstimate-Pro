...
2. Next active product-improvement task: Phase 8 backend route-level Scope-to-Price X-Ray / confirmation construction audit, especially `buildScopeXRay` and `buildAreaScopeBreakdown`.
...
- NEXT: Phase 8 backend route-level Scope-to-Price X-Ray / confirmation construction audit, especially `buildScopeXRay` and `buildAreaScopeBreakdown`. Audit this next because route-level X-Ray, confirmation items, area scope breakdown, and materials/diagnostic construction may still parse raw scope independently.
