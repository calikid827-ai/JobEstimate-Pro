...
2. Next active product-improvement task: Phase 8B audit of `buildMaterialsList` confirmation items/notes and materials diagnostics for remaining raw scope parsing, while keeping `materialsList.items` generation unchanged until a separate scoped task proves it is safe.
