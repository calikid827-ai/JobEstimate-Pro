...
4. NEXT PRODUCT-INTELLIGENCE FEATURE: Photo Intelligence Actions V1.
5. Follow with Plan Intelligence Workflow V1.
...
- Actuals vs Estimate Feedback V1, the bounded Whole-Product Intelligence & Simplification Review, and its focused status-semantics cleanup are complete. Photo Intelligence Actions V1 is the next product-intelligence feature; normal Evidence Authority API, saved estimate, UI, or PDF/customer-output exposure still waits for separate scoped work. Production Live Mode subscription verification remains the final pre-launch gate only.
...
- Phase 8D scope-boundary text/diagnostic cleanup is substantially complete. Actuals vs Estimate Feedback V1, the bounded Whole-Product Intelligence & Simplification Review, and its focused status-semantics cleanup are complete; Photo Intelligence Actions V1 is the next product-intelligence feature. Normal Evidence Authority API, saved estimate, UI, or PDF/customer-output exposure waits for separate scoped work.
...
- Risk level: Low
- Priority: P2
- Recommended fix approach: Keep wording focused on visual support, condition notes, and estimator confirmation. Photo actions belong inside Review Before Sending, must preserve five Command Center sections, should reuse the Scope Decisions interaction pattern instead of leaving a parallel Smart Scope Assist action workflow, and must bind to the generated evidence snapshot and suppress stale actionability after photo changes. Photos remain non-pricing-authoritative; any estimating effect must come through explicit contractor action that changes typed scope followed by successful regeneration.
...
- Runtime actionability currently supports structured material responsibility. Trusted quantity or explicit scope-boundary metadata requires canonical allowlisted subjects and, for quantity, a matching fixed basis/unit. Generic warning, Customer Output Readiness, PriceGuard, Scope X-Ray, missed-scope, plan/photo, or AI prose cannot manufacture deterministic wording; ambiguous ceilings, doors/frames, closets, repairs, access, protection, furniture, occupied-area, and generic-quantity subjects remain deferred without trustworthy metadata.
...
- Deferred whole-product findings remain: Smart Scope Assist / Photo Intelligence consolidation; photo/plan generated-snapshot freshness; optional intake density; Job Workflow progressive disclosure; the duplicate PDF action in Price & Profit; developer-oriented plan transport wording; pre-result PriceGuard chrome; and broad terminology cleanup. Photo workflow consolidation and generated-evidence freshness are required design constraints for Photo Intelligence Actions V1.
