...
1. DONE: Actionable Scope Decisions V1 is complete in `7c5ac91`.
2. DONE: Actuals vs Estimate Feedback V1 is complete in `6f535d2` and `910391d`. It is reference-only and cannot automatically update Rate Card, production rates, pricing formulas, pricing authority, labor totals, or future estimates.
3. DONE: The bounded Whole-Product Intelligence & Simplification Review is complete. Its focused Canonical Contractor Status Semantics Cleanup is complete in `2c8ebb1` without changing readiness logic, invoice calculations, pricing authority, or persistence.
4. DONE: Photo Intelligence Actions V1 is complete in `2e7be1a` and `9832108`.
5. NEXT PRODUCT-INTELLIGENCE FEATURE: Plan Intelligence Workflow V1.
6. Follow with Contractor Learning System after reference-only feedback and evidence workflows are stable.
7. Keep real-world estimate QA, cross-trade scope-boundary filtering, PriceGuard missed-scope checks, Schedule Sequencing Review Guard, and warning-only AI scope protection under regression watch.
8. Final pre-launch gate: complete Production Live Mode subscription payment/webhook entitlement verification before accepting public paid users.
...
- Actuals vs Estimate Feedback V1, the bounded Whole-Product Intelligence & Simplification Review, its focused status-semantics cleanup, and Photo Intelligence Actions V1 are complete. Plan Intelligence Workflow V1 is the next product-intelligence feature; normal Evidence Authority API, saved estimate, UI, or PDF/customer-output exposure still waits for separate scoped work. Production Live Mode subscription verification remains the final pre-launch gate only.
...
- Actuals vs Estimate Feedback V1, the bounded Whole-Product Intelligence & Simplification Review, its focused status-semantics cleanup, and Photo Intelligence Actions V1 are complete. Plan Intelligence Workflow V1 is the next product-intelligence feature; normal Evidence Authority API, saved estimate, UI, or PDF/customer-output exposure still waits for separate scoped work. Prompts, `effectiveScopeChange`, `result.text`, photo pricing behavior, and route/API shape must not change without a scoped review. Production Live Mode subscription verification remains the final pre-launch gate only.
...
- Phase 8D scope-boundary text/diagnostic cleanup is substantially complete. Actuals vs Estimate Feedback V1, the bounded Whole-Product Intelligence & Simplification Review, its focused status-semantics cleanup, and Photo Intelligence Actions V1 are complete; Plan Intelligence Workflow V1 is the next product-intelligence feature. Normal Evidence Authority API, saved estimate, UI, or PDF/customer-output exposure waits for separate scoped work.
...
#### Item: Clarify photo support and consolidate evidence-bound actions
...
- Risk level: Low
- Priority: P2
- Completed approach: Photo Intelligence Actions V1A (`2e7be1a`) and V1B (`9832108`) keep photo actions inside Scope Decisions in Review Before Sending and preserve the five-section Command Center. Scope Decisions is the sole contractor photo-action path; the former Proposal Smart Scope Assist Apply/direct-Regenerate controls are removed while analysis and readbacks remain.
- Exact structured painting-only flags for ceiling and trim/baseboard/casing boundaries may produce actions only when the exact ordered Generate-request evidence is verified current. Stale or unverified evidence, generic photo/AI prose, quantities, measurements, damage/repair/prep/material findings, and mixed plan/photo conclusions remain non-actionable.
- Ordinary Scope Decisions retain trade/subject precedence and the existing three-decision cap. Contractor choices use canonical wording, explicit Apply changes only typed scope, and manual regeneration remains required. Changed request-eligible photo evidence suppresses prior actionability and uses canonical Send Readiness `Regenerate before sending` semantics.
- Evidence provenance is session-only. Loaded saved readbacks remain unverified and non-actionable until successful regeneration; no localStorage, saved/history, API, server, database, or schema field was added. Photos do not directly change pricing, Rate Card, labor totals, production rates, proposal/PDF content, or future estimates.
- Exact files/components involved: `app/app/page.tsx`, `app/app/lib/photo-intelligence-actions.ts`, `app/app/lib/photo-intelligence-action-integration.ts`, and focused helper tests.
- What not to touch: Photo analysis backend, pricing authority, PDFs unless separately scoped.
- Validation: V1A passed TypeScript, 58/58 focused tests, and final review. V1B passed TypeScript and 81/81 focused tests after its narrow no-source Change Order lifecycle correction; short final verification found no issues. Deterministic browser QA with `test12345@gmail.com` used two intentional successful Generate responses, confirmed zero Generate requests from Apply, preserved price/Rate Card/generated proposal content during Apply, required manual regeneration, suppressed stale metadata-changed actions, kept loaded saved estimates unverified, retained five sections and collapsed Advanced Diagnostics, and reported zero substantive runtime/request errors.
- Status: Done for V1. Measured photo takeoff, quantity/measurement pricing authority, reference-scaled takeoff, repair pricing, photo/plan reconciliation, and autonomous learning remain deferred.
...
- Proposal Readiness Badge V1 (`37c8a14`) adds a compact `data-no-print` Send Readiness summary inside Proposal above or near the unchanged delivery actions. It shows one status, one short contractor-facing reason, and optional Review items without creating a sixth section or repeating Customer Output Readiness, PriceGuard Review, Smart Questions, or Advanced Diagnostics details.
- Readiness summarizes existing unsupported-scope drift, Customer Output Readiness items, Estimator Review Summary status, PriceGuard level/score, all unresolved high-priority Smart Questions, and plan/photo warnings. Review items scrolls only to Review Before Sending, does not open diagnostics or call Generate, and does not mutate pricing, `result.text`, history, jobs, invoices, Rate Card, Saved Job Templates, Field Handoff, or Active Job Summary. No readiness state is saved or server-backed, and PDF/copy remain available.
- Severity correction `0afc82e` keeps Unsupported trade wording critical and the scope-overpromise reason higher priority. Customer-ready review remains a normal review signal but is no longer mislabeled critical. Ready and PriceGuard thresholds, helper priority, item generation, Estimator Review Summary, Smart Questions, and plan/photo behavior were unchanged.
- Actionable Scope Decisions V1 (`7c5ac91`) replaces Quick Clarifications in the existing Review Before Sending position. It renders zero to three contractor-facing decisions from safely structured Smart Question metadata and does not create another Command Center section, diagnostics panel, saved state, or pricing authority.
- Runtime actionability remains structured and allowlist-only. Material responsibility continues, and Photo Intelligence Actions V1 adds only exact verified-current painting flags for ceiling and trim/baseboard/casing boundaries. Generic warning, Customer Output Readiness, PriceGuard, Scope X-Ray, missed-scope, plan input, arbitrary photo/AI prose, quantities, measurements, doors/frames, closets, repairs, access, protection, furniture, and occupied-area findings cannot manufacture deterministic wording.
- A contractor selects a choice, previews the exact deterministic sentence, and explicitly applies it to typed scope. Selection does not mutate scope. Apply changes only typed scope, does not call Generate, leaves proposal/pricing/labor/PDF-copy/workflow readbacks based on the generated-scope snapshot, and changes Send Readiness to `Regenerate before sending` until successful manual regeneration clears stale/transient state.
- Send Readiness counts all unresolved high-priority Smart Questions, including review-only questions without an Apply interaction. The reason says high-priority review item rather than Scope Decision. Resolved boundary statements do not independently create an unresolved `scope_boundary` Smart Question; each candidate review entry must contain both a boundary subject and explicit unresolved/action-needed language. Suggested exclusions and the generic Assumptions / exclusions wrapper are not independent triggers, and entries are evaluated individually to prevent cross-entry contamination. Resolved exclusions may still appear elsewhere in proposal or review workflows; this rule governs only creation of the unresolved boundary Smart Question.
- Feature wording uses exact normalized line/sentence duplicate checks and session-only ownership. Replacement requires an unchanged post-Apply scope snapshot plus the exact recorded sentence range. Any manual textarea edit fails closed with feedback, leaves contractor prose unchanged, and appends nothing. Owner-supplied wording makes no pricing assertion, and trusted quantity wording cannot expose arbitrary caller/AI labels.
- Price & Profit contains Rate Card V1, editable pricing controls, and existing PDF behavior.
- Job Workflow contains Active Job Summary, Jobs, Job Templates, Invoices, Saved Estimates, and Field Handoff V1.
...
- Active Job Summary was added in `489351c Add job workflow summary` at the top of the existing Job Workflow section. It does not create a sixth Command Center section and keeps Jobs, Job Templates, Invoices, Saved Estimates, and Field Handoff below it.
- Active Job Summary derives contractor-facing workflow information from existing state only: active job, current loaded estimate only when it matches the summarized job, job details, workflow/pipeline status, contract value, approval status, invoice summary, recorded-cost workflow status, crew load, and Field Handoff readiness where available. It adds no localStorage key, server persistence, saved estimate/job/invoice data shape, pricing authority, PDF content, API response shape, or `/api/generate` behavior.
- Active Job Summary provides one practical next action, such as Create/select job, Copy approval link, Create deposit invoice, Create final/balance invoice, Open invoices, Open actuals, or Open Field Handoff. Summary actions reuse existing nearby workflow behavior or scroll to existing workflow surfaces; summary-only scroll/open actions do not call Generate and do not mutate pricing, `result.text`, history, jobs, or invoices.
- Actuals vs Estimate Feedback V1 is complete across helper V1A (`6f535d2`) and contractor-facing Job Workflow V1B (`910391d`). It uses only the exact nonblank original saved estimate and same-job contractor-entered actuals, compares Labor, Materials, and Subs / other costs to date, leaves missing or invalid values as Not recorded, and never falls back to another estimate or uses invoice, approval, revision, or change-order data.
- The compact `data-no-print` comparison appears only in the active Job Workflow card above the existing actual-cost editor. It is reference-only and does not calculate actual profit, final margin, completion, efficiency, billing variance, or recommendations; it does not call Generate or mutate Rate Card, production rates, pricing, proposals/PDFs, history, invoices, or future estimates. Existing actual-cost storage keys and shapes remain unchanged.
- The selected-job partial-cost Profit Protection / live-margin readback and visible Actuals / Profit summary tile were removed because recorded costs to date are not final profit inputs. This correction is limited to that Job Workflow contradiction and does not claim every profit or margin surface elsewhere in the product was audited.
- Deferred beyond V1: invoice/billing and collections comparisons, actual labor hours, physical completion, equipment-specific actuals, final actual profit, final margin, production efficiency, automatic Rate Card learning, and automatic future-estimate changes.
- The bounded Whole-Product Intelligence & Simplification Review is complete. Its two contractor-facing status findings were closed in `2c8ebb1 Clarify contractor status semantics`: Proposal Send Readiness remains the sole send/no-send verdict, while the clean Estimator Review headline is the panel-scoped `No additional estimator review items.` Readiness thresholds, priorities, inputs, and authority did not change.
- Paid invoice workflow now displays `Paid` rather than `Paid / Closed`, so payment no longer visibly implies physical completion. The payment-specific messages remain `Deposit and balance have both been paid.` and `Final invoice has been paid.` The internal `paid_closed` value remains an intentionally retained compatibility/routing key rather than a persisted completion state. JobEstimate Pro still does not infer job completion from invoice payment.
- Validation for the status cleanup passed TypeScript, 78/78 focused tests, final focused review, and browser regression with `test12345@gmail.com`. The five-section Command Center and collapsed Advanced Diagnostics architecture remained intact, and the cleanup changed no pricing, authority, invoice calculation, or persistence behavior.
- Photo Intelligence Actions V1 completed the Smart Scope Assist / Photo Intelligence contractor-action consolidation and photo generated-evidence freshness work. Deferred whole-product findings remain: plan generated-snapshot freshness; optional intake density; Job Workflow progressive disclosure; the duplicate PDF action in Price & Profit; developer-oriented plan transport wording; pre-result PriceGuard chrome; and broad terminology cleanup.
...
- Full no-code end-to-end regression QA passed after `37c8a14 Add proposal readiness badge` using `test12345@gmail.com`.
- QA verified `/api/generate` returned 200; five sections rendered; Advanced Diagnostics collapsed by default; Send Readiness appeared inside Proposal; Review items was scroll-only without Generate or checked-state mutation; Proposal PDF/copy, Rate Card, Active Job Summary/cross-job guard, Field Handoff copy isolation, and Job Templates persistence/apply remained working; intended print controls hid; no console/page errors or regressions appeared; the dev server stopped; and the working tree stayed clean. Browser QA did not naturally reach Ready because upstream review signals remained; helper tests cover the clean Ready state, and a complete code-level painting case produced strong PriceGuard with no warning/risk rows.
- Focused QA after `0afc82e` showed `Review before sending` with `4 pre-send items need review.`, no inaccurate critical wording, no extra Generate or checked localStorage mutation, working PDF/copy actions, no console/page errors, and a stopped dev server. The fixture contained neither Customer-ready review nor unsupported scope drift, so those branches were confirmed through code inspection and existing helper behavior.
- Post-push regression after `7c5ac91` passed `npx tsc --noEmit`, 54/54 focused tests, and browser QA with `test12345@gmail.com`. Both intentional Generate requests returned 200; exactly five sections remained; one safe material decision rendered without unsupported inferred subjects; stale/readback/ownership/regeneration/PDF-copy/print/mobile checks passed; no console/page errors appeared; all processes stopped; and the working tree remained clean.
- Actuals vs Estimate Feedback V1 passed TypeScript, 40/40 focused tests, focused review, and browser regression with `test12345@gmail.com`. Exactly five Command Center sections remained; sequential Labor, Materials, Notes, and Subs / other input retained focus; actual-cost edits made no Generate request; pricing, Rate Card, proposal, history, and invoice state remained unchanged; mobile/print checks passed; and no console, React, hydration, or page errors appeared.
...
## Phase 9 Evidence Authority / Estimate Basis Readback

#### Item: Phase 9A internal Evidence Authority helper

- Problem: The estimator brain had typed-scope, photo, plan, and deterministic basis data, but no single helper-level classifier for evidence source and authority.
- Why it matters: Future measured takeoff and plan/photo pricing handoff need a clear distinction between pricing-authoritative inputs, review-only evidence, diagnostic-only evidence, boundary context, and future measured-takeoff candidates.
- Risk level: Low
- Priority: P1
- Recommended fix approach: Completed the first internal/helper-level readback model only. `buildEvidenceAuthorityReadback()` classifies typed included scope, typed boundary/exclusion scope, user quantities, parsed quantities, deterministic estimate basis, photo observations, photo quantity signals, plan sheet evidence, plan tables/finish schedules, repeated room packages, plan quantity candidates, and future measured plan quantities.
- Exact files/components involved: `app/api/generate/lib/estimator/evidenceAuthority.ts`, `app/api/generate/lib/estimator/evidenceAuthority.test.ts`.
- What not to touch: `/api/generate` wiring, route/API response shape, pricing, prompts, `result.text`, materials generation, `scopeSplitter`, deterministic engines, `detectTradeStack`, `buildComplexityProfile`, plan candidate gates, photo pricing policy, billing/auth, UI, or deployment.
- Tests or manual QA needed: Completed with evidence authority tests, EstimatorScopeFacts tests, photo decision tests, plan candidate gate tests, estimator review tests, `npx tsc --noEmit`, and `git diff --check`.
- Status: Done

Done note:

- Phase 9A was helper-level/internal only and was not wired into runtime estimate generation.
- No pricing, prompt, customer result text, route/API shape, materials, deterministic engine, billing, UI, or deployment behavior changed.
- Plan quantity candidates remain non-pricing-authoritative.
- Photo observations remain review-only.
- Photo quantities only become pricing-authoritative when explicitly marked as already-authoritative through an existing guarded path.

#### Item: Phase 9B Evidence Authority readback/display wiring

- Problem: Phase 9B-internal now builds Evidence Authority inside the orchestrator and Phase 9B-debug exposes it only for debug requests, but no normal API, saved estimate, UI, or PDF/customer-safe readback surface exposes it yet.
...
#### Item: Phase 9B-debug Evidence Authority debug-only payload exposure

- Problem: Evidence Authority was built internally, but developers had no route-level visibility without changing normal `/api/generate` response shape.
- Why it matters: Debug-only visibility helps verify estimate-basis evidence before any contractor-facing response, saved estimate, UI, or PDF/customer-output shape change.
- Risk level: Low
- Priority: P1
- Recommended fix approach: Completed debug-only route exposure. `/api/generate` captures `EvidenceAuthorityReadback` through `onEvidenceAuthorityReadback` only when debug mode is enabled by `x-debug: 1` / `wantsDebug(req)`. Normal responses return the original payload and do not include `evidenceAuthorityReadback`; debug responses may include it.
- Exact files/components involved: `app/api/generate/route.ts`, `app/api/generate/lib/estimator/orchestratorEstimateSections.test.ts`.
- What not to touch: Normal route/API response shape, `EstimatorPayload`, saved estimate shape, UI, PDFs/customer output, pricing, prompts, `effectiveScopeChange`, `result.text`, materials generation, deterministic engines, `scopeSplitter`, `detectTradeStack`, `buildComplexityProfile`, plan/photo pricing authority, billing/auth, deployment, or customer-facing semantics.
- Tests or manual QA needed: Completed with evidence authority tests, orchestrator/debug-envelope tests, EstimatorScopeFacts tests, photo decision tests, plan candidate gate tests, estimator review tests, `npx tsc --noEmit`, and `git diff --check`. Known note: no direct route integration test exists for `app/api/generate/route.ts`, so route behavior was verified by code inspection plus the focused orchestrator/debug-envelope test.
- Status: Done

Done note:

- Debug requests bypass idempotency cache read/write so debug payloads cannot be cached and replayed later as normal responses, and normal cached payloads cannot suppress debug Evidence Authority readback.
- Normal `/api/generate` response shape remains unchanged.
- `EstimatorPayload`, saved estimate shape, UI, PDFs/customer output, pricing totals, `pricingSource`, pricing owner behavior, pricing formulas, prompts, `effectiveScopeChange`, `result.text`, materials generation, deterministic engines, billing/auth, deployment, and customer-facing semantics were unchanged.
...
#### Item: Phase 9B-estimator-display Evidence Authority exposure audit

- Problem: Evidence Authority now exists internally and in debug-only responses, but exposing it beyond debug would intentionally change normal API response, saved estimate, UI, and possibly PDF/customer-output assumptions.
...
- Risk level: Medium
- Priority: P1
- Recommended fix approach: Run an audit/planning pass first. Decide whether Evidence Authority belongs in a new diagnostics field, an existing estimator diagnostics surface, saved estimate history, or a later customer-safe display. Do not change pricing, prompts, `result.text`, customer-facing output semantics, or plan/photo pricing authority.
...
- This panel was formerly labeled `Quick Clarifications`; Actionable Scope Decisions V1 now replaces that interaction in the same Review Before Sending position while the underlying Smart Question builder continues to supply structured review questions.
...
- Tests or manual QA needed: Completed with Crew Planning tests 7/7 for V1, Crew Planning tests 9/9 and Customer Scope Drift tests 74/74 for the focused flooring-protection / Crew Planning display polish, Crew Planning tests 14/14 for the multi-trade clarity polish, and Crew Planning tests 18/18 for the V2 daily work-plan. `npx tsc --noEmit` and `git diff --check` passed. Final desktop/mobile Playwright QA passed with `test12345@gmail.com`; artifacts were saved outside the repo at `/tmp/jobestimatepro-crew-planning-v2-daily-plan-qa/final-run-5`.
- Status: Done
...
- Multi-trade clarity polish in commit `Clarify multi-trade crew planning` added `planningNotes` and a compact estimator-only note inside the Crew Planning panel when typed scope clearly includes multiple trades, such as painting plus flooring or painting plus outlets. Protection-only floor wording such as protect floors with drop cloths, floor protection, flooring protected with drop cloths, and drop cloths over flooring does not trigger the note.
- V2 daily work-plan polish in commit `Add crew planning daily work plan` added `dailyPlan`, `dailyPlanNotes`, and `dailyPlanConfidence`. Daily Work Plan renders inside the existing Crew Planning panel only, below crew options and above Work Sequence; no new top-level panel was added. Each item includes `label`, `crewSize`, `tasks`, `reminders`, `risks`, and `guidanceOnly: true`. Simple painting, dry-time painting, flooring protection, true painting/flooring, painting/electrical, and hotel/multi-unit scenarios passed desktop/mobile Playwright QA. Hotel/multi-unit uses rolling production entries rather than room-level cards; true flooring/electrical work still surfaces clearly. Optional later polish: simple painting may show Visit 1 / Visit 2 when generated schedule reports 2 visits even without explicit return-trip wording; it is readable but slightly less calm than a single Day 1.
