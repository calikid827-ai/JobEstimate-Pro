...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8D: audit remaining route-level customer-facing diagnostics / prompt-adjacent scope summaries for raw scope parsing. This must be audit/planning only first because prompts and `result.text` must not change without a scoped review; keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8D: audit remaining route-level customer-facing diagnostics / prompt-adjacent scope summaries for raw scope parsing, not billing. Keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. Prioritize Phase 8D customer-facing diagnostics / prompt-adjacent scope summaries audit as the next smart-estimator task. Keep final Production Live Mode subscription verification pending before public paid launch.
