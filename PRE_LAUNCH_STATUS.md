...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8B: audit `buildMaterialsList` confirmation items/notes and materials diagnostics for remaining raw scope parsing. Keep `materialsList.items` generation unchanged unless a later scoped task proves it is safe, and keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8B: audit `buildMaterialsList` confirmation items/notes and materials diagnostics for remaining raw scope parsing, not billing. Keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. Prioritize Phase 8B `buildMaterialsList` confirmation items/notes and materials diagnostics audit as the next smart-estimator task. Keep final Production Live Mode subscription verification pending before public paid launch.
