...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8D follow-up audit of remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `profitLeakDetector` and photo-estimate decision text. Keep prompts, `effectiveScopeChange`, `result.text`, pricing policy, photo pricing behavior, and route/API shape unchanged unless a later scoped task explicitly changes them; keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8D follow-up audit of remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `profitLeakDetector` and photo-estimate decision text, not billing. Keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. Prioritize the Phase 8D follow-up audit of remaining diagnostic/text paths that still use raw scope, `tradeStack`, or `complexityProfile`, especially `profitLeakDetector` and photo-estimate decision text, as the next smart-estimator task. Keep final Production Live Mode subscription verification pending before public paid launch.
