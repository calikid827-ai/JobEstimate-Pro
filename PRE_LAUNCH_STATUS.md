...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8C: audit remaining route-level `materialsList.items` generation for raw scope parsing. This must be audit/planning only first because item generation is customer-visible and higher risk; keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. The next active smart-estimator task is Phase 8C: audit remaining route-level `materialsList.items` generation for raw scope parsing, not billing. Keep Production Live Mode subscription verification as the final pre-launch gate.
...
- NEXT: Work from `PRODUCT_AUDIT_BACKLOG.md`. Prioritize Phase 8C remaining route-level `materialsList.items` generation audit as the next smart-estimator task. Keep final Production Live Mode subscription verification pending before public paid launch.
