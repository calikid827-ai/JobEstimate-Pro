...
- Post-push regression QA passed after `7c5ac91 Add actionable scope decisions` using `test12345@gmail.com`.
- Actionable Scope Decisions validation passed `npx tsc --noEmit` and 54/54 focused tests. Browser QA returned 200 for both intentional Generate requests; preserved exactly five Command Center sections; rendered one safe material-responsibility decision without inferred ceiling, repair, access, protection, or occupied-area decisions; kept generated-result readbacks frozen before regeneration; passed stale readiness, immediate replacement, fail-closed ownership, regeneration, PDF/copy, print, and mobile checks; reported no console/page errors; stopped all processes; and left the working tree clean.
- Actuals vs Estimate Feedback V1 validation passed TypeScript, 40/40 focused tests, focused review, and browser regression for the implementation later committed as `910391d Add actuals vs estimate workflow feedback`, using `test12345@gmail.com`. Exactly five Command Center sections remained; sequential Labor, Materials, Notes, and Subs / other input retained focus; actual-cost edits made no Generate request; Rate Card, pricing, proposal, history, and invoice state remained unchanged; mobile/print checks passed; and no console, React, hydration, or page errors appeared.
...
- Scope Decisions replaces Quick Clarifications in its existing Review Before Sending position. Quick Clarifications must not also render, zero through three decisions are valid, and Scope Decisions must not create another Command Center section or diagnostics panel.
- Scope Decisions uses only safely structured Smart Question metadata. Structured material responsibility is currently actionable at runtime; quantity or explicit scope boundaries require allowlisted subject metadata plus a trusted basis/fixed unit where applicable. Generic warning, diagnostic, PriceGuard, Customer Output Readiness, Scope X-Ray, missed-scope, plan/photo, or AI prose must not manufacture deterministic wording. Ambiguous ceilings, doors/frames, closets, repairs, access, protection, furniture moving, occupied-area coordination, and generic quantity remain deferred without trustworthy metadata.
- Selecting a decision shows the exact deterministic sentence without changing typed scope or calling Generate. `Apply to typed scope` changes only typed scope and makes Send Readiness show `Regenerate before sending`; proposal text, pricing/labor, review/readiness details, Crew Planning, Field Handoff, Active Job Summary, PDF, copied proposal text, and saved workflow data must remain based on the generated-scope snapshot until successful manual regeneration.
- Immediate replacement is allowed only while the post-Apply scope snapshot and recorded sentence range remain exact. Any manual textarea edit must produce ownership-loss feedback, leave contractor prose unchanged, append nothing, call no Generate request, and keep stale readiness. Existing exact contractor wording must be treated as already present without being claimed; partial wording embedded in a longer sentence is not an exact duplicate.
- Successful manual regeneration must update generated-result readbacks, clear stale readiness and transient Scope Decision state, and evaluate Send Readiness normally. Failed generation must not clear the prior snapshot or transient state.
...
- Send Readiness shows only one status, one short contractor-facing reason, and optional Review items. It summarizes existing unsupported-scope drift, Customer Output Readiness items, Estimator Review Summary status, PriceGuard level/score, all unresolved high-priority Smart Questions, and plan/photo warnings without duplicating their details or exposing Advanced Diagnostics content. Both actionable decisions and high-priority review-only questions prevent Ready, and the reason uses `high-priority review item` because not every item has an Apply interaction.
- Proposal Send Readiness is the only send/no-send verdict. A clean Estimator Review says `No additional estimator review items.` even when a separate high-priority Smart Question correctly keeps Send Readiness at `Review before sending`; Estimator Review must not present a second readiness verdict. A fully clean canonical case may still show `Ready to send` in Proposal.
...
- A review-only scope-boundary question is created only when one review entry contains both a boundary subject and explicit unresolved/action-needed language. Resolved statements such as `Ceilings are excluded.`, `Doors and frames are by others.`, `No allowances are included.`, `Existing trim is to remain.`, and `Owner will supply finish paint.` must not create a blocker. Suggested exclusions and the generic Assumptions / exclusions wrapper are not independent triggers, and entries are evaluated individually.
- Price & Profit contains Rate Card V1 and editable pricing controls.
- Job Workflow contains Active Job Summary, Jobs/dashboard, Actuals vs Estimate, Job Templates, Invoices, Saved Estimates, and Field Handoff V1. Actuals vs Estimate stays inside the active job card and must not create a sixth Command Center section.
- Active Job Summary appears near the top of Job Workflow without creating a sixth Command Center section.
- Active Job Summary derives contractor-facing workflow information from existing state only: active job, current loaded estimate only when it matches the summarized job, job details, workflow/pipeline status, contract value, approval status, invoice summary, recorded-cost workflow status, crew load, and Field Handoff readiness when available. It must not present partial recorded costs as actual profit or live margin.
- Paid invoice workflow surfaces display `Paid` and must not infer `Closed`, `Complete`, or `Completed` from payment alone. Payment status may coexist with Actuals vs Estimate stating that job completion is unknown.
...
- Actuals vs Estimate uses only the exact linked original saved estimate. A missing, blank, or unresolved `originalEstimateId` shows `Original saved estimate not available for this job.` and must not select another estimate or hide the actual-cost editor.
- A linked comparison renders exactly Labor, Materials, and Subs / other in that order. Missing or invalid recorded values show Not recorded rather than a confirmed `$0.00`; partial recorded costs remain reference-only; and all supported costs recorded shows that costs are recorded to date, job completion is unknown, and final profit is not calculated.
- Comparison wording is limited to Over estimate, Under estimate, On estimate, Unplanned recorded cost, and Not recorded. It must not show actual profit, final margin, completion, efficiency, invoice/billing conclusions, recommendations, or automatic learning claims.
- Editing Labor, Materials, Subs / other, or Notes uses the existing immediate update path, keeps the editor open, supports normal multi-character typing without focus loss, and updates the comparison without a Save button or `/api/generate` request. Rate Card, estimate pricing, typed scope, proposal/PDF content, saved estimate history, and invoices remain unchanged.
- The reference-only statement says the comparison has not changed Rate Card or future estimates. The Actuals vs Estimate surface is `data-no-print` and must not appear in customer-facing proposal print/PDF output.
...
- Print mode hides Send Readiness, Scope Decisions, Proposal delivery controls, Rate Card, Job Templates, Actuals vs Estimate, Active Job Summary workflow-only controls, Field Handoff action controls, and Advanced Diagnostics.
